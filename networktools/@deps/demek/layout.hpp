...
class sugiyama_layout {
  ...
  sugiyama_layout(graph g, attributes attr) : g(g), original_vertex_count(g.size()), attrs(attr) { build(); }
  ...
  void normalize() {
    const float f_denom = size.x > size.y ? size.x : size.y;
    ...
    for (auto& path: paths) {
      for (int i = 0; i < path.points.size(); ++i) {
        path.points[i].x /= f_denom;
        path.points[i].y /= f_denom;
      }
    }
  }
  ...
  std::unique_ptr< detail::cycle_removal > cycle_module = std::make_unique< detail::dfs_removal >();

  std::unique_ptr< detail::layering > layering_module = std::make_unique< detail::network_simplex_layering >();

  std::unique_ptr< detail::crossing_reduction > crossing_module = std::make_unique< detail::barycentric_heuristic >();

  std::unique_ptr< detail::positioning > positioning_module = std::make_unique< detail::fast_and_simple_positioning >(attrs, nodes, boxes);

  std::unique_ptr< detail::edge_router > routing_module = std::make_unique< detail::router >(nodes, paths, attrs);
...
#ifdef CONTROL_CROSSING
    if (crossing_enabled) { crossing->run(h); }
#else
  ...
  void init_nodes() {
    ...
    for (auto u: g.vertices()) {
      ...
      boxes[u] = {{2 * nodes[u].size, 2 * nodes[u].size}, {nodes[u].size, nodes[u].size}};
    }
  }
