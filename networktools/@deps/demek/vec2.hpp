...
template <typename T> inline vec2 operator*(T a, vec2 vec) {
  return {a * vec.x, a * vec.y};
}
template <typename T> inline vec2 operator*(vec2 vec, T a) {
  return {a * vec.x, a * vec.y};
}
...
inline float cross(vec2 u, vec2 v) { return u.x * v.y - u.y * v.x; }
...
inline float line_point_dist(vec2 from, vec2 to, vec2 p) {
  auto v = to - from;
  auto w = p - from;
...
inline std::optional<vec2> line_circle_intersection(vec2 from, vec2 to,
                                                    vec2 center, float r) {
  auto d = to - from;
  auto f = from - center;
