...
#  undef MAX
  ...
  */
  LOGURU_EXPORT
  void add_callback(
     const char* id, log_handler_t callback, void* user_data, Verbosity verbosity, close_handler_t on_close = nullptr, flush_handler_t on_flush = nullptr);
...
#  if LOGURU_USE_FMTLIB
  ...
  LOGURU_NORETURN void
     vlog_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, fmt::format_args);
  template < typename... Args >
...
#  else
  LOGURU_EXPORT
  LOGURU_NORETURN void log_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, ...)
    ...
    do {                                                                                                                                      \
      ...
      if (!LOGURU_PREDICT_TRUE(val_left op val_right)) {                                                                                      \
        ...
        auto fail_info = loguru::textprintf(                                                                                                  \
           "CHECK FAILED:  " LOGURU_FMT(s) " " LOGURU_FMT(s) " " LOGURU_FMT(s) "  (" LOGURU_FMT(s) " " LOGURU_FMT(s) " " LOGURU_FMT(s) ")  ", \
           #expr_left,                                                                                                                        \
           #op,                                                                                                                               \
           #expr_right,                                                                                                                       \
           str_left.c_str(),                                                                                                                  \
...
#    define DEFINE_CHECK_OP_IMPL(name, op)                                                         \
      ...
      inline std::string* name(const char* expr, int v1, const char* op_str, int v2) { return name< int, int >(expr, v1, op_str, v2); }
...
#    define CHECK_OP_S(function_name, expr1, op, expr2)                                                                                        \
      while (auto error_string =                                                                                                               \
                loguru::function_name(#expr1 " " #op " " #expr2, loguru::referenceable_value(expr1), #op, loguru::referenceable_value(expr2))) \
...
#  endif
  ...
  inline Verbosity g_stderr_verbosity = Verbosity_0;
  inline bool      g_colorlogtostderr = true;
  inline unsigned  g_flush_interval_ms = 0;
  inline bool      g_preamble_header = true;
  inline bool      g_preamble = true;

  inline Verbosity g_internal_verbosity = Verbosity_0;
  ...
  inline bool g_preamble_verbose = true;
  inline bool g_preamble_pipe = true;
...
#  else
    ...
    if (const char* term = getenv("TERM")) {
      return 0 == strcmp(term, "cygwin") || 0 == strcmp(term, "linux") || 0 == strcmp(term, "rxvt-unicode-256color") || 0 == strcmp(term, "screen")
             || 0 == strcmp(term, "screen-256color") || 0 == strcmp(term, "screen.xterm-256color") || 0 == strcmp(term, "tmux-256color")
             || 0 == strcmp(term, "xterm") || 0 == strcmp(term, "xterm-256color") || 0 == strcmp(term, "xterm-termite") || 0 == strcmp(term, "xterm-color");
    } else {
      return false;
    }
#  endif
  ...
  inline bool terminal_has_color() { return s_terminal_has_color; }
...
#  endif
  ...
  inline const char* terminal_black() { return s_terminal_has_color ? VTSEQ(30) : ""; }
  inline const char* terminal_red() { return s_terminal_has_color ? VTSEQ(31) : ""; }
  inline const char* terminal_green() { return s_terminal_has_color ? VTSEQ(32) : ""; }
  inline const char* terminal_yellow() { return s_terminal_has_color ? VTSEQ(33) : ""; }
  inline const char* terminal_blue() { return s_terminal_has_color ? VTSEQ(34) : ""; }
  inline const char* terminal_purple() { return s_terminal_has_color ? VTSEQ(35) : ""; }
  inline const char* terminal_cyan() { return s_terminal_has_color ? VTSEQ(36) : ""; }
  inline const char* terminal_light_gray() { return s_terminal_has_color ? VTSEQ(37) : ""; }
  inline const char* terminal_white() { return s_terminal_has_color ? VTSEQ(37) : ""; }
  inline const char* terminal_light_red() { return s_terminal_has_color ? VTSEQ(91) : ""; }
  inline const char* terminal_dim() { return s_terminal_has_color ? VTSEQ(2) : ""; }

  // Formating
  inline const char* terminal_bold() { return s_terminal_has_color ? VTSEQ(1) : ""; }
  inline const char* terminal_underline() { return s_terminal_has_color ? VTSEQ(4) : ""; }

  // You should end each line with this!
  inline const char* terminal_reset() { return s_terminal_has_color ? VTSEQ(0) : ""; }
...
#  if LOGURU_WITH_FILEABS
  void file_reopen(void* user_data) {
    ...
    if (!file_abs->fp || (ret = stat(file_abs->path, &st)) == -1 || (st.st_ino != file_abs->st.st_ino)) {
      ...
      if (!create_directories(file_abs->path)) { LOG_F(ERROR, "Failed to create directories to '" LOGURU_FMT(s) "'", file_abs->path); }
...
#  endif
  ...
  // ------------------------------------------------------------------------------
#  if LOGURU_SYSLOG
  inline void syslog_log(void* /*user_data*/, const Message& message) {
    /*
  ...
  inline void syslog_close(void* /*user_data*/) { closelog(); }

  inline void syslog_flush(void* /*user_data*/) {}
#  endif
  // ------------------------------------------------------------------------------
  // Helpers:

  inline Text::~Text() { free(_str); }

#  if LOGURU_USE_FMTLIB
  inline Text vtextprintf(const char* format, fmt::format_args args) { return Text(STRDUP(fmt::vformat(format, args).c_str())); }
#  else
...
#  endif
  ...
  // Overloaded for variadic template matching.
  inline Text textprintf() { return Text(static_cast< char* >(calloc(1, 1))); }
...
#  endif
  ...
  static long long now_ns() { return duration_cast< nanoseconds >(high_resolution_clock::now().time_since_epoch()).count(); }
...
#  endif
  ...
  inline void init(int& argc, char* argv[], const Options& options) {
...
#    if !defined(__EMSCRIPTEN__) && (defined(__APPLE__) || defined(__linux__) || defined(__sun))
...
#  endif   // LOGURU_PTHREADS
  ...
  inline void write_date_time(char* buff, unsigned long long buff_size) {
  ...
  inline const char* argv0_filename() { return s_argv0_filename.c_str(); }

  inline const char* arguments() { return s_arguments.c_str(); }

  inline const char* current_dir() { return s_current_dir; }

  inline const char* home_dir() {
#  ifdef __MINGW32__
...
#  endif   // _WIN32
  ...
  inline void suggest_log_path(const char* prefix, char* buff, unsigned long long buff_size) {
    if (prefix[0] == '~') {
...
#  endif
  ...
  inline bool create_directories(const char* file_path_const) {
...
#  endif
  ...
  inline bool add_file(const char* path_in, FileMode mode, Verbosity verbosity) {
    char path[PATH_MAX];
    if (path_in[0] == '~') {
...
#  endif
  ...
  */
  inline bool add_syslog(const char* app_name, Verbosity verbosity) { return add_syslog(app_name, verbosity, LOG_USER); }
  inline bool add_syslog(const char* app_name, Verbosity verbosity, int facility) {
#  if LOGURU_SYSLOG
...
#  endif
  ...
  // Will be called right before abort().
  inline void set_fatal_handler(fatal_handler_t handler) { s_fatal_handler = handler; }

  inline fatal_handler_t get_fatal_handler() { return s_fatal_handler; }

  inline void set_verbosity_to_name_callback(verbosity_to_name_t callback) { s_verbosity_to_name_callback = callback; }

  inline void set_name_to_verbosity_callback(name_to_verbosity_t callback) { s_name_to_verbosity_callback = callback; }

  inline void add_stack_cleanup(const char* find_this, const char* replace_with_this) {
  ...
  inline void add_callback(const char* id, log_handler_t callback, void* user_data, Verbosity verbosity, close_handler_t on_close, flush_handler_t on_flush) {
  ...
  inline const char* get_verbosity_name(Verbosity verbosity) {
  ...
  inline Verbosity get_verbosity_from_name(const char* name) {
  ...
  inline void remove_all_callbacks() {
    std::lock_guard< std::recursive_mutex > lock(s_mutex);
  ...
  // Returns the maximum of g_stderr_verbosity and all file/custom outputs.
  inline Verbosity current_verbosity_cutoff() { return g_stderr_verbosity > s_max_out_verbosity ? g_stderr_verbosity : s_max_out_verbosity; }
...
#  if LOGURU_PTLS_NAMES
  ...
  inline void make_pthread_key_name() { (void)pthread_key_create(&s_pthread_key_name, free); }
#  endif

#  if LOGURU_WINTHREADS
  // Where we store the custom thread name set by `set_thread_name`
  inline char* thread_name_buffer() {
...
#  endif   // LOGURU_PTHREADS
  ...
  inline void get_thread_name(char* buffer, unsigned long long length, bool right_align_hex_id) {
...
#  if LOGURU_STACKTRACES
  inline Text demangle(const char* name) {
    int   status = -1;
...
#    endif   // LOGURU_RTTI
  ...
  inline void do_replacements(const StringPairList& replacements, std::string& str) {
  ...
  inline std::string prettify_stacktrace(const std::string& input) {
...
#  else   // LOGURU_STACKTRACES
  inline Text demangle(const char* name) { return Text(STRDUP(name)); }
...
#  endif   // LOGURU_STACKTRACES
  ...
  static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line) {
    ...
    if (g_preamble_date && pos < out_buff_size) {
      int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%04d-%02d-%02d ", 1900 + time_info.tm_year, 1 + time_info.tm_mon, time_info.tm_mday);
      if (bytes > 0) { pos += bytes; }
    }
    if (g_preamble_time && pos < out_buff_size) {
      int bytes =
         snprintf(out_buff + pos, out_buff_size - pos, "%02d:%02d:%02d.%03lld ", time_info.tm_hour, time_info.tm_min, time_info.tm_sec, ms_since_epoch % 1000);
      if (bytes > 0) { pos += bytes; }
    }
    if (g_preamble_uptime && pos < out_buff_size) {
    ...
    if (g_preamble_file && pos < out_buff_size) {
      ...
      int bytes = snprintf(out_buff + pos, out_buff_size - pos, "%*s:%-5u ", LOGURU_FILENAME_WIDTH, shortened_filename, line);
      if (bytes > 0) { pos += bytes; }
    }
    if (g_preamble_verbose && pos < out_buff_size) {
...
#  endif
  ...
  // stack_trace_skip is just if verbosity == FATAL.
  inline void log_to_everywhere(int stack_trace_skip, Verbosity verbosity, const char* file, unsigned line, const char* prefix, const char* buff) {
...
#  if LOGURU_USE_FMTLIB
  inline void vlog(Verbosity verbosity, const char* file, unsigned line, const char* format, fmt::format_args args) {
    auto formatted = fmt::vformat(format, args);
    log_to_everywhere(1, verbosity, file, line, "", formatted.c_str());
  }

  inline void raw_vlog(Verbosity verbosity, const char* file, unsigned line, const char* format, fmt::format_args args) {
    auto formatted = fmt::vformat(format, args);
    auto message = Message{verbosity, file, line, "", "", "", formatted.c_str()};
    log_message(1, message, false, true);
  }
#  else
  inline void log(Verbosity verbosity, const char* file, unsigned line, const char* format, ...) {
    va_list vlist;
    va_start(vlist, format);
  ...
  inline void vlog(Verbosity verbosity, const char* file, unsigned line, const char* format, va_list vlist) {
    auto buff = vtextprintf(format, vlist);
    log_to_everywhere(1, verbosity, file, line, "", buff.c_str());
  }

  inline void raw_log(Verbosity verbosity, const char* file, unsigned line, const char* format, ...) {
    va_list vlist;
    va_start(vlist, format);
    auto buff = vtextprintf(format, vlist);
...
#  endif
  ...
  inline LogScopeRAII::LogScopeRAII(Verbosity verbosity, const char* file, unsigned line, const char* format, va_list vlist) :
      _verbosity(verbosity), _file(file), _line(line) {
    this->Init(format, vlist);
  }

  inline LogScopeRAII::LogScopeRAII(Verbosity verbosity, const char* file, unsigned line, const char* format, ...) :
      _verbosity(verbosity), _file(file), _line(line) {
    va_list vlist;
    va_start(vlist, format);
    this->Init(format, vlist);
  ...
  inline LogScopeRAII::~LogScopeRAII() {
    if (_file) {
      std::lock_guard< std::recursive_mutex > lock(s_mutex);
...
#  endif
  ...
  inline void LogScopeRAII::Init(const char* format, va_list vlist) {
...
#  if LOGURU_USE_FMTLIB
  inline void vlog_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, const char* format, fmt::format_args args) {
    auto formatted = fmt::vformat(format, args);
...
#  else
  inline std::string vstrprintf(const char* format, va_list vlist) {
...
#  endif   // LOGURU_WITH_STREAMS
  ...
  inline void stream_print(StringStream& out_string_stream, const char* text) { out_string_stream.str += text; }
...
#    endif
  ...
  inline ECPtr& get_thread_ec_head_ref() { return thread_ec_ptr; }
#  else    // !thread_local
  static pthread_once_t s_ec_pthread_once = PTHREAD_ONCE_INIT;
  static pthread_key_t s_ec_pthread_key;

  inline void free_ec_head_ref(void* io_error_context) { delete reinterpret_cast< ECPtr* >(io_error_context); }

  inline void ec_make_pthread_key() { (void)pthread_key_create(&s_ec_pthread_key, free_ec_head_ref); }

  inline ECPtr& get_thread_ec_head_ref() {
...
#  endif   // !thread_local
  ...
  inline EcHandle get_thread_ec_handle() { return get_thread_ec_head_ref(); }

  inline Text get_error_context() { return get_error_context_for(get_thread_ec_head_ref()); }

  inline Text get_error_context_for(const EcEntryBase* ec_head) {
...
#  endif
  ...
  inline EcEntryBase::EcEntryBase(const char* file, unsigned line, const char* descr) : _file(file), _line(line), _descr(descr) {
    EcEntryBase*& ec_head = get_thread_ec_head_ref();
    _previous = ec_head;
    ec_head = this;
  }

  inline EcEntryBase::~EcEntryBase() { get_thread_ec_head_ref() = _previous; }

  // ------------------------------------------------------------------------

  inline Text ec_to_text(const char* value) {
...
#  define DEFINE_EC(Type)                \
    inline Text ec_to_text(Type value) { \
...
#  undef DEFINE_EC
  ...
  inline Text ec_to_text(EcHandle ec_handle) {
...
namespace loguru {
  ...
  inline void write_to_stderr(const char* data) { write_to_stderr(data, strlen(data)); }
  ...
  inline void signal_handler(int signal_number, siginfo_t*, void*) {
  ...
  inline void install_signal_handlers(const SignalOptions& signal_options) {
    ...
    if (signal_options.sigabrt) { CHECK_F(sigaction(SIGABRT, &sig_action, NULL) != -1, "Failed to install handler for SIGABRT"); }
    if (signal_options.sigbus) { CHECK_F(sigaction(SIGBUS, &sig_action, NULL) != -1, "Failed to install handler for SIGBUS"); }
    if (signal_options.sigfpe) { CHECK_F(sigaction(SIGFPE, &sig_action, NULL) != -1, "Failed to install handler for SIGFPE"); }
    if (signal_options.sigill) { CHECK_F(sigaction(SIGILL, &sig_action, NULL) != -1, "Failed to install handler for SIGILL"); }
    if (signal_options.sigint) { CHECK_F(sigaction(SIGINT, &sig_action, NULL) != -1, "Failed to install handler for SIGINT"); }
    if (signal_options.sigsegv) { CHECK_F(sigaction(SIGSEGV, &sig_action, NULL) != -1, "Failed to install handler for SIGSEGV"); }
    if (signal_options.sigterm) { CHECK_F(sigaction(SIGTERM, &sig_action, NULL) != -1, "Failed to install handler for SIGTERM"); }
  }
}   // namespace loguru
