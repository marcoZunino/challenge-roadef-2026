 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (dheap.h).
 *
 ...
 * @tparam D The degree of the heap, each node have at most \e D
 ...
 */
  ...
  class DHeap {
    ...
    private:
    constexpr int parent(int i) const noexcept { return (i - 1) / D; }
    constexpr int firstChild(int i) const noexcept { return D * i + 1; }
    ...
    constexpr void bubbleDown(int hole, Pair p, int length) noexcept {
      if (length > 1) {
        ...
        while (child + D <= length) {
          int min = child;
          for (int i = 1; i < D; ++i) {
            if (less(_data[child + i], _data[min])) min = child + i;
          }
          if (!less(_data[min], p)) {
            move(p, hole);
            return;
