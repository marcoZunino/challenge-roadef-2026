...
#ifndef _NT_CONCEPT_MATRIX_MAPS_H_
#define _NT_CONCEPT_MATRIX_MAPS_H_
...
namespace nt {
  ...
  namespace concepts {
    ...
    class ReadMatrixMap {
      ...
      Value operator()(const FirstKey&, const SecondKey&) const { return Value(); }
      ...
      struct Constraints {
        void constraints() {
          ...
          typename _ReadMatrixMap::Value own_val = m(own_first_key, own_second_key);
    ...
    template < typename K1, typename K2, typename V >
    class ReadWriteMatrixMap: public ReadMatrixMap< K1, K2, V >, public WriteMatrixMap< K1, K2, V > {
      public:
      /** Map's first key type. */
      typedef K1 FirstKey;
      /** Map's second key type. */
    ...
    class ReferenceMatrixMap: public ReadWriteMatrixMap< K1, K2, V > {
       ...
       */
      Reference operator()(const FirstKey&, const SecondKey&) { return tmp; }
      /**
       * Returns a const reference to the value associated to a pair of keys.
       */
      ConstReference operator()(const FirstKey&, const SecondKey&) const { return tmp; }
      /** Sets the value associated with the pair of keys. */
      void set(const FirstKey&, const SecondKey&, const Value&) {}
