 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (fib_heap.h).
 *
...
#ifndef _NT_FIB_HEAP_H_
#define _NT_FIB_HEAP_H_
 ...
 */
  ...
  class FibHeap {
     ...
     */
    FibHeap(ItemIntMap& map, const Compare& comp) : _minimum(0), _iim(map), _comp(comp), _num() {}
     ...
     */
    constexpr void push(const Item& item, const Prio& prio) noexcept {
      int i = _iim[item];
      if (i < 0) {
        int s = _data.size();
        _iim.set(item, s);
     ...
     */
    constexpr Item top() const noexcept { return _data[_minimum].name; }
     ...
     * @pre The heap must be non-empty.
     */
    constexpr Prio prio() const noexcept { return _data[_minimum].prio; }
     ...
     * @pre \e item must be in the heap.
     */
    constexpr Prio operator[](const Item& item) const noexcept { return _data[_iim[item]].prio; }
     ...
     */
    constexpr void set(const Item& item, const Prio& prio) noexcept {
      int i = _iim[item];
      if (i >= 0 && _data[i].in) {
     ...
     */
    constexpr void decrease(const Item& item, const Prio& prio) noexcept {
      int i = _iim[item];
      _data[i].prio = prio;
      int p = _data[i].parent;
     ...
     */
    constexpr void increase(const Item& item, const Prio& prio) noexcept {
      erase(item);
      push(item, prio);
    }
    ...
    void balance() noexcept {
      ...
      nt::TrivialDynamicArray< int > A(maxdeg, -1);
    ...
    constexpr void makeRoot(int c) noexcept {
      int s = c;
      do {
        _data[s].parent = -1;
        s = _data[s].right_neighbor;
    ...
    constexpr void cut(int a, int b) noexcept {
      /*
      ...
      if (_data[b].degree != 0) {
        ...
        if (child == a) _data[b].child = _data[child].right_neighbor;
        unlace(a);
      }
    ...
    constexpr void cascade(int a) noexcept {
      if (_data[a].parent != -1) {
