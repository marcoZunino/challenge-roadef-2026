 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (bucket_heap.h).
 *
...
#ifndef _NT_BUCKET_HEAP_H_
#define _NT_BUCKET_HEAP_H_
...
namespace nt {
   ...
   * @tparam MIN Indicate if the heap is a \e min-heap or a \e max-heap.
  ...
  class BucketHeap {
    ...
    private:
    using Direction = _bucket_heap_bits::DirectionTraits< MIN >;
    ...
    private:
    constexpr void relocateLast(int idx) noexcept {
      if (idx + 1 < int(_data.size())) {
        ...
        if (_data[idx].next != -1) { _data[_data[idx].next].prev = idx; }
        _iim[_data[idx].item] = idx;
      }
      _data.pop_back();
    }

    constexpr void unlace(int idx) noexcept {
      if (_data[idx].prev != -1) {
        _data[_data[idx].prev].next = _data[idx].next;
      } else {
        _first[_data[idx].value] = _data[idx].next;
      }
      if (_data[idx].next != -1) { _data[_data[idx].next].prev = _data[idx].prev; }
    }

    constexpr void lace(int idx) noexcept {
      if (int(_first.size()) <= _data[idx].value) { _first.resize(_data[idx].value + 1, -1); }
     ...
     */
    constexpr void push(const Pair& p) noexcept { push(p.first, p.second); }
    ...
    constexpr void push(const Item& i, const Prio& p) noexcept {
      ...
      lace(idx);
      if (Direction::less(p, _minimum)) { _minimum = p; }
    }
    ...
    ItemIntMap&                           _iim;
    nt::TrivialDynamicArray< int >        _first;
    nt::TrivialDynamicArray< BucketItem > _data;
  ...
  class SimpleBucketHeap {
     ...
     * The assigned value must be \c PRE_HEAP (<tt>-1</tt>) for each item.
     */
    explicit SimpleBucketHeap(ItemIntMap& map) : _iim(map), _free(-1), _num(0), _minimum(0) {}
