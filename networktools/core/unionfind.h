 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (unionfind.h).
 *
...
#ifndef _NT_UNION_FIND_H_
#define _NT_UNION_FIND_H_
...
namespace nt {
  ...
  class UnionFind {
    ...
    IntDynamicArray items;
    ItemIntMap&     index;

    constexpr bool rep(int idx) const noexcept { return items[idx] < 0; }

    constexpr int repIndex(int idx) const noexcept {
      int k = idx;
      while (!rep(k)) {
        k = items[k];
      }
     ...
     *
     */
    constexpr int find(const Item& a) noexcept { return repIndex(index[a]); }
     ...
     */
    constexpr void clear() noexcept { items.clear(); }
     ...
     */
    constexpr int insert(const Item& a) noexcept {
      int n = items.size();
      items.push_back(-1);
      index.set(a, n);
      return n;
     ...
     */
    constexpr bool join(const Item& a, const Item& b) noexcept {
     ...
     */
    constexpr int size(const Item& a) noexcept {
  ...
  class UnionFindEnum {
    ...
    nt::TrivialDynamicArray< ItemT > items;
    int                              firstFreeItem;
    ...
    nt::TrivialDynamicArray< ClassT > classes;
    int                               firstClass, firstFreeClass;

    constexpr int newClass() noexcept {
      if (firstFreeClass == -1) {
    ...
    constexpr int newItem() noexcept {
      if (firstFreeItem == -1) {
        int idx = items.size();
    ...
    constexpr bool rep(int idx) const noexcept { return items[idx].parent < 0; }
    ...
    constexpr int classIndex(int idx) const noexcept { return ~(items[repIndex(idx)].parent); }

    constexpr void singletonItem(int idx) noexcept {
      items[idx].next = idx;
      items[idx].prev = idx;
    }

    constexpr void laceItem(int idx, int rdx) noexcept {
      items[idx].prev = rdx;
    ...
    constexpr void unlaceItem(int idx) noexcept {
    ...
    constexpr void spliceItems(int ak, int bk) noexcept {
    ...
    constexpr void laceClass(int cls) noexcept {
      if (firstClass != -1) { classes[firstClass].prev = cls; }
      classes[cls].next = firstClass;
      classes[cls].prev = -1;
      firstClass = cls;
    }

    constexpr void unlaceClass(int cls) noexcept {
      if (classes[cls].prev != -1) {
        classes[classes[cls].prev].next = classes[cls].next;
      } else {
        firstClass = classes[cls].next;
      }
      if (classes[cls].next != -1) { classes[classes[cls].next].prev = classes[cls].prev; }
    ...
    public:
    UnionFindEnum(ItemIntMap& _index) : index(_index), items(), firstFreeItem(-1), firstClass(-1), firstFreeClass(-1) {}
     ...
     *
     */
    constexpr int insert(const Item& item) noexcept {
      int idx = newItem();
     ...
     * element \e comp.
     */
    constexpr void insert(const Item& item, int cls) noexcept {
     ...
     */
    constexpr int find(const Item& item) const noexcept { return ~(items[repIndex(index[item])].parent); }
     ...
     */
    constexpr int join(const Item& a, const Item& b) noexcept {
     ...
     */
    constexpr int size(int cls) const noexcept { return classes[cls].size; }
     ...
     * of size one).
     */
    constexpr void split(int cls) noexcept {
     ...
     */
    constexpr Item item(int cls) const noexcept { return items[classes[cls].firstItem].item; }
     ...
     * structure.
     */
    constexpr void eraseClass(int cls) noexcept {
      int fdx = classes[cls].firstItem;
      unlaceClass(cls);
    ...
    class ClassIt {
       ...
       */
      ClassIt(const UnionFindEnum& ufe) : unionFind(&ufe) { cdx = unionFind->firstClass; }
       ...
       */
      constexpr operator int() const noexcept { return cdx; }
       ...
       * Equality operator
       */
      constexpr bool operator==(const ClassIt& i) noexcept { return i.cdx == cdx; }
    ...
    class ItemIt {
       ...
       */
      ItemIt(const UnionFindEnum& ufe, int cls) : unionFind(&ufe) { fdx = idx = unionFind->classes[cls].firstItem; }
       ...
       */
      constexpr operator const Item&() const noexcept { return unionFind->items[idx].item; }
       ...
       * Equality operator
       */
      constexpr bool operator==(const ItemIt& i) noexcept { return i.idx == idx; }
  ...
  class ExtendFindEnum {
    ...
    ExtendFindEnum(ItemIntMap& _index) :
        index(_index), items(), firstFreeItem(-1), classes(), firstClass(-1), firstFreeClass(-1) {}
    ...
    constexpr int insert(const Item& item) noexcept {
      ...
      if (firstClass != -1) { classes[firstClass].prev = cdx; }
      firstClass = cdx;
     ...
     */
    constexpr int find(const Item& item) const noexcept { return items[index[item]].cls; }
    ...
    constexpr void erase(const Item& item) noexcept {
      ...
      if (idx == items[idx].next) {
        ...
        if (classes[cdx].next != -1) { classes[classes[cdx].next].prev = classes[cdx].prev; }
     ...
     * structure.
     */
    constexpr void eraseClass(int cdx) noexcept {
    ...
    class ClassIt {
       ...
       * Constructor of the iterator
       */
      ClassIt(const ExtendFindEnum& ufe) : extendFind(&ufe) { cdx = extendFind->firstClass; }
       ...
       * It steps to the next representant item.
       */
      constexpr ClassIt& operator++() noexcept {
    ...
    class ItemIt {
       ...
       * on the class of the \c item.
       */
      ItemIt(const ExtendFindEnum& ufe, int cls) : extendFind(&ufe) { fdx = idx = extendFind->classes[cls].firstItem; }
       ...
       * It converts the iterator to the current item.
       */
      constexpr operator const Item&() const noexcept { return extendFind->items[idx].item; }
  ...
  class HeapUnionFind {
    ...
    nt::TrivialDynamicArray< ClassNode > classes;
    ...
    constexpr void deleteClass(int id) noexcept {
    ...
    int                                 first_free_node;
    nt::TrivialDynamicArray< ItemNode > nodes;

    constexpr int newNode() noexcept {
    ...
    constexpr void deleteNode(int id) noexcept {
    ...
    constexpr int findClass(int id) const noexcept {
      int kd = id;
      while (kd >= 0) {
        kd = nodes[kd].parent;
      }
    ...
    constexpr int leftNode(int id) const noexcept {
    ...
    constexpr int nextNode(int id) const noexcept {
      int depth = 0;
    ...
    constexpr void setPrio(int id) noexcept {
    ...
    constexpr void push(int id, int jd) noexcept {
      nodes[id].size = 1;
    ...
    constexpr void pushAfter(int id, int jd) noexcept {
      int kd = nodes[id].parent;
      if (nodes[id].next != -1) {
        nodes[nodes[id].next].prev = jd;
        if (kd >= 0) { nodes[kd].size += 1; }
      } else {
        if (kd >= 0) {
          nodes[kd].right = jd;
          nodes[kd].size += 1;
    ...
    constexpr void pushRight(int id, int jd) noexcept {
      nodes[id].size += 1;
    ...
    constexpr void popRight(int id) noexcept {
      nodes[id].size -= 1;
    ...
    constexpr void splice(int id, int jd) noexcept {
    ...
    constexpr void split(int id, int jd) noexcept {
      int kd = nodes[id].parent;
    ...
    constexpr void pushLeft(int id, int jd) noexcept {
      nodes[id].size += 1;
    ...
    constexpr void popLeft(int id) noexcept {
      nodes[id].size -= 1;
      int jd = nodes[id].left;
    ...
    constexpr void repairLeft(int id) noexcept {
        ...
        if (nodes[jd].size == 1) {
          if (nodes[jd].parent < 0) {
            ...
            if (nodes[nodes[jd].next].size < cmax) {
              ...
              if (nodes[nodes[jd].next].item == nodes[ld].item) { setPrio(nodes[jd].next); }
              jd = nodes[jd].left;
            }
          }
        } else {
    ...
    constexpr void repairRight(int id) noexcept {
      int jd = ~(classes[id].parent);
        ...
        if (nodes[jd].size == 1) {
          if (nodes[jd].parent < 0) {
            ...
            if (nodes[nodes[jd].prev].size < cmax) {
              ...
              if (nodes[nodes[jd].prev].item == nodes[ld].item) { setPrio(nodes[jd].prev); }
              jd = nodes[jd].right;
            }
          }
        } else {
    ...
    constexpr bool less(int id, int jd) const noexcept { return comp(nodes[id].prio, nodes[jd].prio); }
     ...
     */
    constexpr bool alive(int cls) const noexcept { return classes[cls].parent < 0; }
     ...
     */
    constexpr bool trivial(int cls) const noexcept { return classes[cls].left == -1; }
     ...
     */
    HeapUnionFind(ItemIntMap& _index) : index(_index), first_class(-1), first_free_class(-1), first_free_node(-1) {}
     ...
     */
    constexpr int insert(const Item& item, const Value& prio) noexcept {
      int id = newNode();
      nodes[id].item = item;
      nodes[id].prio = prio;
      nodes[id].size = 0;
      ...
      if (first_class != -1) { classes[first_class].prev = class_id; }
     ...
     */
    constexpr int find(const Item& item) const noexcept { return findClass(index[item]); }
    ...
    int join(Iterator begin, Iterator end) noexcept {
      IntDynamicArray cs;
      ...
      {   // creation union-find
        ...
        if (classes[l].next != -1) { classes[classes[l].next].prev = classes[l].prev; }
        ...
        for (int ci = 1; ci < int(cs.size()); ++ci) {
          ...
          if (classes[l].depth > classes[r].depth) {
            ...
            while (id >= 0 && nodes[id].size == cmax) {
              ...
              popRight(id);
              if (nodes[id].item == nodes[right_id].item) { setPrio(id); }
            ...
            while (id >= 0 && nodes[id].size == cmax) {
              ...
              popLeft(id);
              if (nodes[id].prio == nodes[left_id].prio) { setPrio(id); }
              push(new_id, left_id);
            ...
            if (classes[l].depth != 0 && nodes[~(classes[l].parent)].size + nodes[~(classes[r].parent)].size <= cmax) {
              splice(~(classes[l].parent), ~(classes[r].parent));
              deleteNode(~(classes[r].parent));
              if (less(~(classes[r].parent), ~(classes[l].parent))) {
                nodes[~(classes[l].parent)].prio = nodes[~(classes[r].parent)].prio;
                nodes[~(classes[l].parent)].item = nodes[~(classes[r].parent)].item;
              }
            } else {
              int new_parent = newNode();
          ...
          if (classes[r].next != -1) { classes[classes[r].next].prev = classes[r].prev; }
     ...
     */
    template < typename Iterator >
    void split(int cls, Iterator out) noexcept {
      IntDynamicArray cs;
      {   // splitting union-find
        ...
        *out++ = l;
        ...
        if (classes[id].next != -1) { classes[classes[id].next].prev = classes[id].prev; }
     ...
     */
    constexpr const Value& operator[](const Item& item) const noexcept { return nodes[index[item]].prio; }
     ...
     */
    constexpr void set(const Item& item, const Value& prio) noexcept {
     ...
     */
    constexpr void increase(const Item& item, const Value& prio) noexcept {
      int id = index[item];
      int kd = nodes[id].parent;
      nodes[id].prio = prio;
     ...
     * Increase the priority of the current item.
     */
    constexpr void decrease(const Item& item, const Value& prio) noexcept {
      int id = index[item];
      int kd = nodes[id].parent;
      nodes[id].prio = prio;
     ...
     */
    constexpr const Value& classPrio(int cls) const noexcept { return nodes[~(classes[cls].parent)].prio; }
     ...
     */
    constexpr const Item& classTop(int cls) const noexcept { return nodes[~(classes[cls].parent)].item; }
     ...
     * items.
     */
    constexpr const Item& classRep(int id) const noexcept {
    ...
    class ItemIt {
       ...
       * It converts the iterator to the current item.
       */
      constexpr operator const Item&() const noexcept { return _huf->nodes[_id].item; }
       ...
       * Equality operator
       */
      constexpr bool operator==(const ItemIt& i) noexcept { return i._id == _id; }
       ...
       * Equality operator
       */
      constexpr bool operator==(Invalid) noexcept { return _id == _lid; }
    ...
    class ClassIt {
      ...
      ClassIt(const HeapUnionFind& huf, int cls) : _huf(&huf), _id(huf.classes[cls].left) {}

      ClassIt(Invalid) : _huf(0), _id(-1) {}

      constexpr const ClassIt& operator++() noexcept {
       ...
       * Equality operator
       */
      constexpr bool operator==(const ClassIt& i) noexcept { return i._id == _id; }
      ...
      constexpr operator int() const noexcept { return _id; }
    };
  };
