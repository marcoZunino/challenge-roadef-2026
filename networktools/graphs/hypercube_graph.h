 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (hypercube_graph.h).
 *
...
#ifndef _NT_HYPERCUBE_GRAPH_H_
#define _NT_HYPERCUBE_GRAPH_H_
...
namespace nt {
  namespace graphs {
    ...
    class HypercubeGraphBase {
      public:
      using Graph = HypercubeGraphBase;
      ...
      void construct(int dim) {
        NT_ASSERT(dim >= 1, "The number of dimensions must be at least 1.");
        _dim = dim;
        _node_num = 1 << dim;
      ...
      void nextInc(Edge& edge, bool& dir) const {
        ...
        if (k < _dim) {
          edge._id = (k << (_dim - 1)) | ((n._id >> (k + 1)) << k) | (n._id & ((1 << k) - 1));
          dir = ((n._id >> k) & 1) == 0;
        } else {
          edge._id = -1;
          dir = true;
      ...
      void firstOut(Arc& arc, const Node& node) const { arc._id = ((node._id >> 1) << 1) | (~node._id & 1); }

      void nextOut(Arc& arc) const {
        Node n = (arc._id & 1) == 1 ? u(arc) : v(arc);
        int  k = (arc._id >> _dim) + 1;
        if (k < _dim) {
          arc._id = (k << (_dim - 1)) | ((n._id >> (k + 1)) << k) | (n._id & ((1 << k) - 1));
      ...
      void firstIn(Arc& arc, const Node& node) const { arc._id = ((node._id >> 1) << 1) | (node._id & 1); }
      ...
      bool projection(Node node, int n) const { return static_cast< bool >(node._id & (1 << n)); }
    ...
    using ExtendedHypercubeGraphBase = GraphExtender< HypercubeGraphBase >;
     ...
     *
     * @note The type of the indices is chosen to \c int for efficiency
     * reasons. Thus the maximum dimension of this implementation is 26
     * (assuming that the size of \c int is 32 bit).
     */
    class HypercubeGraph: public ExtendedHypercubeGraphBase {
      using Parent = ExtendedHypercubeGraphBase;
       ...
       */
      bool projection(Node node, int n) const { return Parent::projection(node, n); }
      ...
      class HyperMap {
        ...
        HyperMap(const Graph& graph, It begin, It end, T fv = 0, const BF& bf = BF()) :
          ...
          NT_ASSERT(_values.size() == graph.dimension(), "Wrong size of range");
        }
        ...
        Value operator[](const Key& k) const {
          ...
          int   n = 0;
          while (id != 0) {
            if (id & 1) { val = _bin_func(val, _values[n]); }
            id >>= 1;
            ++n;
          }
          return val;
