 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (lgf_writer.h).
 *
...
#ifndef _NT_LGF_WRITER_H_
#define _NT_LGF_WRITER_H_
...
namespace nt {
  ...
  namespace graphs {
    ...
    class IoError: public Exception {
      ...
      /** Constructor */
      explicit IoError(const std::string& message) throw() { IoError::message(message); }
      ...
      /** Constructor */
      explicit IoError(const std::string& message, const std::string& file) throw() {
        IoError::message(message);
        IoError::file(file);
      }
    ...
    class FormatError: public Exception {
      ...
      /** Constructor */
      explicit FormatError(const char* message, const std::string& file, int line = 0) throw() {
        FormatError::message(message);
      ...
      /** Constructor */
      explicit FormatError(const std::string& message, const std::string& file, int line = 0) throw() {
        FormatError::message(message);
        FormatError::file(file);
        FormatError::line(line);
      }
    ...
    namespace _writer_bits {
      ...
      class MapLess {
        ...
        bool operator()(const Item& left, const Item& right) { return _map[left] < _map[right]; }
      };
      ...
      class GraphArcMapLess {
        ...
        using Map = _Map;
        using Graph = _Graph;
        using Item = typename Graph::Edge;
        ...
        public:
        GraphArcMapLess(const Graph& graph, const Map& map) : _graph(graph), _map(map) {}
      ...
      class MapStorage: public MapStorageBase< _Item > {
        ...
        public:
        MapStorage(const Map& map, const Converter& converter = Converter()) : _map(map), _converter(converter) {}
      ...
      class GraphArcMapStorage: public MapStorageBase< typename _Graph::Edge > {
        ...
        public:
        GraphArcMapStorage(const Graph& graph, const Map& map, const Converter& converter = Converter()) :
            _graph(graph), _map(map), _converter(converter) {}
        virtual ~GraphArcMapStorage() noexcept {}

        virtual std::string get(const Item& item) { return _converter(_map[_graph.direct(item, dir)]); }
        virtual void        sort(std::vector< Item >& items) {
      ...
      struct GraphArcLookUpConverter {
        ...
        GraphArcLookUpConverter(const Graph& graph, const std::map< typename Graph::Edge, std::string >& map) :
            _graph(graph), _map(map) {}

        std::string operator()(const typename Graph::Arc& val) {
          typename std::map< typename Graph::Edge, std::string >::const_iterator it = _map.find(val);
          if (it == _map.end()) { throw FormatError("Item not found"); }
      ...
      inline bool isEscaped(char c) { return c == '\\' || c == '\"' || c == '\'' || c == '\a' || c == '\b'; }
      ...
      inline bool requireEscape(const std::string& str) {
        ...
        char               c;
        while (is.get(c)) {
          if (isWhiteSpace(c) || isEscaped(c)) { return true; }
        }
        return false;
      }
    ...
    DigraphWriter< TDGR > digraphWriter(const TDGR& digraph, std::ostream& os = std::cout);
    template < typename TDGR >
    ...
    class DigraphWriter {
      ...
      using NodeIndex = std::map< Node, std::string >;
      NodeIndex _node_index;
      using ArcIndex = std::map< Arc, std::string >;
      ArcIndex _arc_index;

      using NodeMaps = std::vector< std::pair< std::string, _writer_bits::MapStorageBase< Node >* > >;
      NodeMaps _node_maps;

      using ArcMaps = std::vector< std::pair< std::string, _writer_bits::MapStorageBase< Arc >* > >;
      ArcMaps _arc_maps;

      using Attributes = std::vector< std::pair< std::string, _writer_bits::ValueStorageBase* > >;
      ...
      DigraphWriter(const DGR& digraph, std::ostream& os = std::cout) :
          _os(&os), local_os(false), _digraph(digraph), _skip_nodes(false), _skip_arcs(false) {}
      ...
      DigraphWriter(const DGR& digraph, const std::string& fn) :
          _os(new std::ofstream(fn.c_str())), local_os(true), _digraph(digraph), _skip_nodes(false), _skip_arcs(false) {
        if (!(*_os)) {
          delete _os;
      ...
      DigraphWriter(const DGR& digraph, const char* fn) :
          _os(new std::ofstream(fn)), local_os(true), _digraph(digraph), _skip_nodes(false), _skip_arcs(false) {
        if (!(*_os)) {
          delete _os;
          throw IoError("Cannot write file", fn);
        }
      ...
      ~DigraphWriter() noexcept {
        ...
        if (local_os) { delete _os; }
      }

      private:
      template < typename TDGR >
      friend DigraphWriter< TDGR > digraphWriter(const TDGR& digraph, std::ostream& os);
      template < typename TDGR >
      friend DigraphWriter< TDGR > digraphWriter(const TDGR& digraph, const std::string& fn);
      template < typename TDGR >
      friend DigraphWriter< TDGR > digraphWriter(const TDGR& digraph, const char* fn);

      DigraphWriter(DigraphWriter& other) :
          _os(other._os), local_os(other.local_os), _digraph(other._digraph), _skip_nodes(other._skip_nodes),
      ...
      DigraphWriter&
         attribute(const std::string& caption, const Value& value, const Converter& converter = Converter()) {
      ...
      DigraphWriter& node(const std::string& caption, const Node& node) {
        using Converter = _writer_bits::MapLookUpConverter< Node >;
        Converter                       converter(_node_index);
        _writer_bits::ValueStorageBase* storage = new _writer_bits::ValueStorage< Node, Converter >(node, converter);
        _attributes.push_back(std::make_pair(caption, storage));
        return *this;
      }
      ...
      DigraphWriter& arc(const std::string& caption, const Arc& arc) {
        using Converter = _writer_bits::MapLookUpConverter< Arc >;
        Converter                       converter(_arc_index);
        _writer_bits::ValueStorageBase* storage = new _writer_bits::ValueStorage< Arc, Converter >(arc, converter);
        _attributes.push_back(std::make_pair(caption, storage));
        return *this;
      }
      ...
      DigraphWriter& skipNodes() {
        NT_ASSERT(!_skip_nodes, "Multiple usage of skipNodes() member");
        _skip_nodes = true;
        return *this;
      }
      ...
      DigraphWriter& skipArcs() {
        NT_ASSERT(!_skip_arcs, "Multiple usage of skipArcs() member");
        _skip_arcs = true;
        return *this;
      }
      ...
      void writeNodes() {
        ...
        *_os << "@nodes";
        if (!_nodes_caption.empty()) { _writer_bits::writeToken(*_os << ' ', _nodes_caption); }
        *_os << std::endl;

        if (label == 0) { *_os << "label" << '\t'; }
        for (typename NodeMaps::iterator it = _node_maps.begin(); it != _node_maps.end(); ++it) {
        ...
        *_os << std::endl;
        ...
        for (int i = 0; i < static_cast< int >(nodes.size()); ++i) {
          ...
          for (typename NodeMaps::iterator it = _node_maps.begin(); it != _node_maps.end(); ++it) {
            ...
            if (it->first == "label") { _node_index.insert(std::make_pair(n, value)); }
            *_os << '\t';
          }
          *_os << std::endl;
        }
      ...
      void writeArcs() {
        ...
        *_os << "@arcs";
        if (!_arcs_caption.empty()) { _writer_bits::writeToken(*_os << ' ', _arcs_caption); }
        *_os << std::endl;
        ...
        *_os << std::endl;
        ...
        for (int i = 0; i < static_cast< int >(arcs.size()); ++i) {
          Arc a = arcs[i];
          _writer_bits::writeToken(*_os, _node_index.find(_digraph.source(a))->second);
          *_os << '\t';
          _writer_bits::writeToken(*_os, _node_index.find(_digraph.target(a))->second);
          *_os << '\t';
          if (label == 0) {
            std::ostringstream os;
            os << _digraph.id(a);
          ...
          for (typename ArcMaps::iterator it = _arc_maps.begin(); it != _arc_maps.end(); ++it) {
            ...
            _writer_bits::writeToken(*_os, value);
            if (it->first == "label") { _arc_index.insert(std::make_pair(a, value)); }
            *_os << '\t';
          }
          *_os << std::endl;
        }
      ...
      void writeAttributes() {
        ...
        *_os << "@attributes";
        if (!_attributes_caption.empty()) { _writer_bits::writeToken(*_os << ' ', _attributes_caption); }
        *_os << std::endl;
        for (typename Attributes::iterator it = _attributes.begin(); it != _attributes.end(); ++it) {
          _writer_bits::writeToken(*_os, it->first) << ' ';
    ...
    class GraphWriter {
      ...
      using NodeIndex = std::map< Node, std::string >;
      NodeIndex _node_index;
      using EdgeIndex = std::map< Edge, std::string >;
      EdgeIndex _edge_index;

      using NodeMaps = std::vector< std::pair< std::string, _writer_bits::MapStorageBase< Node >* > >;
      NodeMaps _node_maps;

      using EdgeMaps = std::vector< std::pair< std::string, _writer_bits::MapStorageBase< Edge >* > >;
      EdgeMaps _edge_maps;
      ...
      GraphWriter(const GR& graph, std::ostream& os = std::cout) :
          _os(&os), local_os(false), _graph(graph), _skip_nodes(false), _skip_edges(false) {}
      ...
      GraphWriter(const GR& graph, const std::string& fn) :
          _os(new std::ofstream(fn.c_str())), local_os(true), _graph(graph), _skip_nodes(false), _skip_edges(false) {
        if (!(*_os)) {
          delete _os;
          throw IoError("Cannot write file", fn);
        }
      ...
      GraphWriter(const GR& graph, const char* fn) :
          _os(new std::ofstream(fn)), local_os(true), _graph(graph), _skip_nodes(false), _skip_edges(false) {
        if (!(*_os)) {
          delete _os;
          throw IoError("Cannot write file", fn);
        }
      ...
      GraphWriter(GraphWriter& other) :
          _os(other._os), local_os(other.local_os), _graph(other._graph), _skip_nodes(other._skip_nodes),
      ...
      GraphWriter& arcMap(const std::string& caption, const Map& map, const Converter& converter = Converter()) {
        checkConcept< concepts::ReadMap< Arc, typename Map::Value >, Map >();
        _writer_bits::MapStorageBase< Edge >* forward_storage =
           new _writer_bits::GraphArcMapStorage< GR, true, Map, Converter >(_graph, map, converter);
        _edge_maps.push_back(std::make_pair('+' + caption, forward_storage));
        _writer_bits::MapStorageBase< Edge >* backward_storage =
           new _writer_bits::GraphArcMapStorage< GR, false, Map, Converter >(_graph, map, converter);
        _edge_maps.push_back(std::make_pair('-' + caption, backward_storage));
        return *this;
      }
      ...
      GraphWriter& edge(const std::string& caption, const Edge& edge) {
        using Converter = _writer_bits::MapLookUpConverter< Edge >;
        Converter                       converter(_edge_index);
        _writer_bits::ValueStorageBase* storage = new _writer_bits::ValueStorage< Edge, Converter >(edge, converter);
        _attributes.push_back(std::make_pair(caption, storage));
        return *this;
      }
      ...
      GraphWriter& arc(const std::string& caption, const Arc& arc) {
        using Converter = _writer_bits::GraphArcLookUpConverter< GR >;
      ...
      GraphWriter& skipEdges() {
        NT_ASSERT(!_skip_edges, "Multiple usage of skipEdges() member");
        _skip_edges = true;
        return *this;
      }
      ...
      void writeEdges() {
        ...
        *_os << "@edges";
        if (!_edges_caption.empty()) { _writer_bits::writeToken(*_os << ' ', _edges_caption); }
        *_os << std::endl;
        ...
        for (int i = 0; i < static_cast< int >(edges.size()); ++i) {
          ...
          *_os << '\t';
          ...
          for (typename EdgeMaps::iterator it = _edge_maps.begin(); it != _edge_maps.end(); ++it) {
            ...
            _writer_bits::writeToken(*_os, value);
            if (it->first == "label") { _edge_index.insert(std::make_pair(e, value)); }
            *_os << '\t';
          }
          *_os << std::endl;
        }
    ...
    BpGraphWriter< TBGR > bpGraphWriter(const TBGR& graph, std::ostream& os = std::cout);
    template < typename TBGR >
    ...
    class BpGraphWriter {
      ...
      using RedNodeIndex = std::map< Node, std::string >;
      RedNodeIndex _red_node_index;
      using BlueNodeIndex = std::map< Node, std::string >;
      BlueNodeIndex _blue_node_index;
      using EdgeIndex = std::map< Edge, std::string >;
      EdgeIndex _edge_index;

      using RedNodeMaps = std::vector< std::pair< std::string, _writer_bits::MapStorageBase< RedNode >* > >;
      RedNodeMaps _red_node_maps;
      using BlueNodeMaps = std::vector< std::pair< std::string, _writer_bits::MapStorageBase< BlueNode >* > >;
      ...
      template < typename TBGR >
      friend BpGraphWriter< TBGR > bpGraphWriter(const TBGR& graph, const std::string& fn);
      template < typename TBGR >
      ...
      BpGraphWriter& nodeMap(const std::string& caption, const Map& map) {
        checkConcept< concepts::ReadMap< Node, typename Map::Value >, Map >();
        _writer_bits::MapStorageBase< RedNode >* red_storage = new _writer_bits::MapStorage< RedNode, Map >(map);
        _red_node_maps.push_back(std::make_pair(caption, red_storage));
        _writer_bits::MapStorageBase< BlueNode >* blue_storage = new _writer_bits::MapStorage< BlueNode, Map >(map);
      ...
      BpGraphWriter& redNodeMap(const std::string& caption, const Map& map) {
        ...
        _writer_bits::MapStorageBase< RedNode >* storage = new _writer_bits::MapStorage< RedNode, Map >(map);
      ...
      BpGraphWriter& blueNodeMap(const std::string& caption, const Map& map) {
        ...
        _writer_bits::MapStorageBase< BlueNode >* storage = new _writer_bits::MapStorage< BlueNode, Map >(map);
      ...
      BpGraphWriter& arcMap(const std::string& caption, const Map& map, const Converter& converter = Converter()) {
        checkConcept< concepts::ReadMap< Arc, typename Map::Value >, Map >();
        _writer_bits::MapStorageBase< Edge >* forward_storage =
           new _writer_bits::GraphArcMapStorage< BGR, true, Map, Converter >(_graph, map, converter);
        _edge_maps.push_back(std::make_pair('+' + caption, forward_storage));
        _writer_bits::MapStorageBase< Edge >* backward_storage =
           new _writer_bits::GraphArcMapStorage< BGR, false, Map, Converter >(_graph, map, converter);
        _edge_maps.push_back(std::make_pair('-' + caption, backward_storage));
        return *this;
      }
      ...
      BpGraphWriter& node(const std::string& caption, const Node& node) {
        using Converter = _writer_bits::DoubleMapLookUpConverter< Node, RedNodeIndex, BlueNodeIndex >;
      ...
      BpGraphWriter& arc(const std::string& caption, const Arc& arc) {
        using Converter = _writer_bits::GraphArcLookUpConverter< BGR >;
        Converter                       converter(_graph, _edge_index);
        _writer_bits::ValueStorageBase* storage = new _writer_bits::ValueStorage< Arc, Converter >(arc, converter);
        _attributes.push_back(std::make_pair(caption, storage));
        return *this;
        ...
        *_os << std::endl;
        ...
        for (int i = 0; i < static_cast< int >(nodes.size()); ++i) {
          ...
          for (typename RedNodeMaps::iterator it = _red_node_maps.begin(); it != _red_node_maps.end(); ++it) {
            std::string value = it->second->get(n);
            _writer_bits::writeToken(*_os, value);
            if (it->first == "label") { _red_node_index.insert(std::make_pair(n, value)); }
            *_os << '\t';
          }
          *_os << std::endl;
        }
        ...
        *_os << std::endl;
        ...
        for (int i = 0; i < static_cast< int >(nodes.size()); ++i) {
          ...
          for (typename BlueNodeMaps::iterator it = _blue_node_maps.begin(); it != _blue_node_maps.end(); ++it) {
            std::string value = it->second->get(n);
            _writer_bits::writeToken(*_os, value);
            if (it->first == "label") { _blue_node_index.insert(std::make_pair(n, value)); }
            *_os << '\t';
          }
          *_os << std::endl;
        }
      ...
      void writeEdges() {
        ...
        *_os << std::endl;
        ...
        for (int i = 0; i < static_cast< int >(edges.size()); ++i) {
          Edge e = edges[i];
          _writer_bits::writeToken(*_os, _red_node_index.find(_graph.redNode(e))->second);
          *_os << '\t';
          _writer_bits::writeToken(*_os, _blue_node_index.find(_graph.blueNode(e))->second);
          *_os << '\t';
          if (label == 0) {
            std::ostringstream os;
            os << _graph.id(e);
    ...
    class SectionWriter {
      ...
      using Sections = std::vector< std::pair< std::string, _writer_bits::Section* > >;
       ...
       */
      SectionWriter(const std::string& fn) : _os(new std::ofstream(fn.c_str())), local_os(true) {
        if (!(*_os)) {
          delete _os;
          throw IoError("Cannot write file", fn);
        }
      ...
      SectionWriter(SectionWriter& other) : _os(other._os), local_os(other.local_os) {
        other._os = 0;
        other.local_os = false;
      ...
      SectionWriter& sectionLines(const std::string& type, Functor functor) {
        ...
        _sections.push_back(std::make_pair(type, new _writer_bits::LineSection< Functor >(functor)));
        return *this;
      }
      ...
      SectionWriter& sectionStream(const std::string& type, Functor functor) {
        NT_ASSERT(!type.empty(), "Type is empty.");
        _sections.push_back(std::make_pair(type, new _writer_bits::StreamSection< Functor >(functor)));
        return *this;
      }
       ...
       */
      void run() {
        NT_ASSERT(_os != 0, "This writer is assigned to an other writer");
