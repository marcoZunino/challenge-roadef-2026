 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (full_graph.h).
 *
...
#ifndef _NT_FULL_GRAPH_H_
#define _NT_FULL_GRAPH_H_
...
namespace nt {
  namespace graphs {
    ...
    class FullDigraphBase {
      ...
      Arc arc(const Node& s, const Node& t) const { return Arc(s._id * _node_num + t._id); }
      ...
      Arc findArc(Node s, Node t, Arc prev = INVALID) const { return prev == INVALID ? arc(s, t) : INVALID; }
      ...
      void firstOut(Arc& arc, const Node& node) const { arc._id = (node._id + 1) * _node_num - 1; }
      ...
      void firstIn(Arc& arc, const Node& node) const { arc._id = _arc_num + node._id - _node_num; }
    ...
    using ExtendedFullDigraphBase = DigraphExtender< FullDigraphBase >;
     ...
     *
     * @note FullDigraph and FullGraph classes are very similar,
    ...
    class FullDigraph: public ExtendedFullDigraphBase {
      using Parent = ExtendedFullDigraphBase;
    ...
    class FullGraphBase {
      ...
      Node source(Arc arc) const { return Node((arc._id & 1) == 1 ? _uid(arc._id >> 1) : _vid(arc._id >> 1)); }

      Node target(Arc arc) const { return Node((arc._id & 1) == 1 ? _vid(arc._id >> 1) : _uid(arc._id >> 1)); }

      using FindEdgeTag = True;
      using FindArcTag = True;

      Edge findEdge(Node u, Node v, Edge prev = INVALID) const { return prev != INVALID ? INVALID : edge(u, v); }

      Arc findArc(Node s, Node t, Arc prev = INVALID) const { return prev != INVALID ? INVALID : arc(s, t); }
    ...
    using ExtendedFullGraphBase = GraphExtender< FullGraphBase >;
    ...
    class FullGraph: public ExtendedFullGraphBase {
      using Parent = ExtendedFullGraphBase;
    ...
    class FullBpGraphBase {
      ...
      BlueNode blueNode(Edge e) const { return BlueNode(e._id / _red_num + _red_num); }
    ...
    using ExtendedFullBpGraphBase = BpGraphExtender< FullBpGraphBase >;
    ...
    class FullBpGraph: public ExtendedFullBpGraphBase {
      public:
      using Parent = ExtendedFullBpGraphBase;
