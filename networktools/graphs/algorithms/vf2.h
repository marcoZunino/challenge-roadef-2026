 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (vf2.h).
 *
      ...
      namespace vf2 {
        ...
        class MapEq {
          ...
          bool operator()(typename M1::Key k1, typename M2::Key k2) const {
            if constexpr (std::is_class< typename M1::Value >::value || std::is_union< typename M1::Value >::value) {
              return (_m1[k1]).operator==(_m2[k2]);
            } else {
        ...
        class BfsLeaveOrder: public BfsVisitor< G > {
          int                                          i;
          const G&                                     _g;
          nt::TrivialDynamicArray< typename G::Node >& _order;

          public:
          BfsLeaveOrder(const G& g, nt::TrivialDynamicArray< typename G::Node >& order) : i(0), _g(g), _order(order) {}
 ...
 * @tparam NEQ A bool-valued binary functor determinining whether a node is
 ...
 */
    ...
    class Vf2 {
      ...
      bool cut(const typename G1::Node n1, const typename G2::Node n2) const {
        ...
        switch (MT) {
          case MappingType::INDUCED: return _rInOut1t[n1] <= rInOut2 && _rNew1t[n1] <= rNew2;
          case MappingType::SUBGRAPH: return _rInOut1t[n1] <= rInOut2;
          case MappingType::ISOMORPH: return _rInOut1t[n1] == rInOut2 && _rNew1t[n1] == rNew2;
      ...
      bool feas(const typename G1::Node n1, const typename G2::Node n2) {
        ...
        for (typename G2::IncEdgeIt e2(_g2, n2); e2 != INVALID; ++e2) {
          ...
          if (connCurrNode < -1)
            ++connCurrNode;
          else if (MT != MappingType::SUBGRAPH && connCurrNode == -1) {
            isIso = 0;
            break;
          }
        }
        ...
        for (typename G1::IncEdgeIt e1(_g1, n1); e1 != INVALID; ++e1) {
          ...
          if (connCurrNodePair != -1) {
            switch (MT) {
              case MappingType::INDUCED:
              case MappingType::ISOMORPH: isIso = 0; break;
              case MappingType::SUBGRAPH:
      ...
      void initRNew1tRInOut1t() {
        ...
        for (unsigned int i = 0; i < _order.size(); ++i) {
          ...
          for (typename G1::IncEdgeIt e1(_g1, orderI); e1 != INVALID; ++e1) {
            const typename G1::Node currNode = _g1.oppositeNode(orderI, e1);
            if (tmp[currNode] != -1) ++tmp[currNode];
          }
        }
      }
      ...
      Vf2(const G1& g1, const G2& g2, M& m, const NEQ& neq = NEQ()) :
          _g1(g1), _g2(g2), _nEq(neq), _depth(0), _mapping(m), _order(nt::graphs::countNodes(g1), INVALID),
          _conn(g2, 0), _currEdgeIts(nt::graphs::countNodes(g1), INVALID), _rNew1t(g1, 0), _rInOut1t(g1, 0),
          _mapping_type(MappingType::SUBGRAPH), _deallocMappingAfterUse(0) {
        initOrder();
        initRNew1tRInOut1t();
    ...
    class Vf2WizardBase {
      ...
      using NodeEq = bits::vf2::AlwaysEq;
      NodeEq _node_eq;

      Vf2WizardBase(const G1& g1, const G2& g2) :
          _g1(g1), _g2(g2), _mapping_type(MappingType::SUBGRAPH), _local_mapping(true) {}
    };
    ...
    class Vf2Wizard: public TR {
      ...
      using NodeEq = typename TR::NodeEq;
      ...
      struct SetNodeEqBase: public Base {
        using NodeEq = NE;
        NodeEq _node_eq;
        SetNodeEqBase(const Base& b, const NE& node_eq) : Base(b), _node_eq(node_eq) {}
      };
       ...
       */
      template < class M1, class M2 >
      Vf2Wizard< SetNodeEqBase< bits::vf2::MapEq< M1, M2 > > > nodeLabels(const M1& m1, const M2& m2) {
      ...
      bool run() {
        ...
        Vf2< Graph1, Graph2, Mapping, NodeEq > alg(_g1, _g2, *reinterpret_cast< Mapping* >(_mapping), _node_eq);
      ...
      Vf2< Graph1, Graph2, Mapping, NodeEq >* getPtrToVf2Object() {
        ...
        Vf2< Graph1, Graph2, Mapping, NodeEq >* ptr =
           new Vf2< Graph1, Graph2, Mapping, NodeEq >(_g1, _g2, *reinterpret_cast< Mapping* >(_mapping), _node_eq);
      ...
      int count() {
        ...
        Vf2< Graph1, Graph2, Mapping, NodeEq > alg(_g1, _g2, *reinterpret_cast< Mapping* >(_mapping), _node_eq);
        if (Base::_local_mapping) alg._deallocMappingAfterUse = true;
        alg.mappingType(_mapping_type);
