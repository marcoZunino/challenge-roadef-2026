 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (min_cost_arborescence.h).
 *
...
#ifndef _NT_MIN_COST_ARBORESCENCE_H_
#define _NT_MIN_COST_ARBORESCENCE_H_
...
namespace nt {
  namespace graphs {
    ...
    struct MinCostArborescenceDefaultTraits {
       ...
       */
      using Value = typename CostMap::Value;
       ...
       */
      using ArborescenceMap = typename Digraph::template ArcMap< bool >;
       ...
       */
      static ArborescenceMap* createArborescenceMap(const Digraph& digraph) { return new ArborescenceMap(digraph); }
       ...
       * \c PredMap.
       */
      static PredMap* createPredMap(const Digraph& digraph) { return new PredMap(digraph); }
    };
 ...
 */
    ...
    class MinCostArborescence {
      public:
      /**
       * @brief The @ref nt::MinCostArborescenceDefaultTraits "traits class"
       * of the algorithm.
       */
      using Traits = TR;
      ...
      /** The type of the map that stores the arc costs. */
      using CostMap = typename Traits::CostMap;
       ...
       */
      using ArborescenceMap = typename Traits::ArborescenceMap;

      using Create = MinCostArborescence;
      ...
      using ArcOrder = typename Digraph::template ArcMap< int >;
      ArcOrder* _arc_order;

      using NodeOrder = typename Digraph::template NodeMap< int >;
      NodeOrder* _node_order;

      using CostArcMap = typename Digraph::template NodeMap< CostArc >;
      CostArcMap* _cost_arcs;

      struct StackLevel {
        nt::TrivialDynamicArray< CostArc > arcs;
        int                                node_level;
      };

      nt::DynamicArray< StackLevel >  level_stack;
      nt::TrivialDynamicArray< Node > queue;

      using DualNodeList = nt::TrivialDynamicArray< typename Digraph::Node >;
      ...
      struct DualVariable {
        ...
        DualVariable(int _begin, int _end, Value _value) : begin(_begin), end(_end), value(_value) {}
      };

      using DualVariables = nt::TrivialDynamicArray< DualVariable >;
      ...
      using Heap = BinHeap< int, HeapCrossRef >;
      ...
      void createStructures() {
        ...
        if (!_arc_order) { _arc_order = new ArcOrder(*_digraph); }
        if (!_node_order) { _node_order = new NodeOrder(*_digraph); }
        if (!_cost_arcs) { _cost_arcs = new CostArcMap(*_digraph); }
        if (!_heap_cross_ref) { _heap_cross_ref = new HeapCrossRef(*_digraph, -1); }
        if (!_heap) { _heap = new Heap(*_heap_cross_ref); }
      }

      void destroyStructures() {
        if (local_arborescence) { delete _arborescence; }
        if (local_pred) { delete _pred; }
        if (_arc_order) { delete _arc_order; }
        if (_node_order) { delete _node_order; }
        if (_cost_arcs) { delete _cost_arcs; }
        if (_heap) { delete _heap; }
        if (_heap_cross_ref) { delete _heap_cross_ref; }
      }
      ...
      Arc prepare(Node node) {
        ...
        for (int i = 1; i < int(nodes.size()); ++i) {
          if ((*_cost_arcs)[nodes[i]].value < minimum.value) { minimum = (*_cost_arcs)[nodes[i]]; }
        }
        (*_arc_order)[minimum.arc] = _dual_variables.size();
        DualVariable var(_dual_node_list.size() - 1, _dual_node_list.size(), minimum.value);
        ...
        level_stack.push_back(std::move(level));
        return minimum.arc;
      }
      ...
      Arc contract(Node node) {
        ...
        nt::TrivialDynamicArray< Node > nodes;
        while (!level_stack.empty() && level_stack.back().node_level >= node_bottom) {
      ...
      struct SetArborescenceMapTraits: public Traits {
        using ArborescenceMap = T;
        static ArborescenceMap* createArborescenceMap(const Digraph&) {
          NT_ASSERT(false, "ArborescenceMap is not initialized");
       ...
       */
      template < class T >
      struct SetArborescenceMap: public MinCostArborescence< Digraph, CostMap, SetArborescenceMapTraits< T > > {};
       ...
       */
      template < class T >
      struct SetPredMap: public MinCostArborescence< Digraph, CostMap, SetPredMapTraits< T > > {};
      ...
      MinCostArborescence(const Digraph& digraph, const CostMap& cost) :
          _digraph(&digraph), _cost(&cost), _pred(0), local_pred(false), _arborescence(0), local_arborescence(false),
          _arc_order(0), _node_order(0), _cost_arcs(0), _heap_cross_ref(0), _heap(0) {}

      /** @brief Destructor. */
      ~MinCostArborescence() noexcept { destroyStructures(); }
       ...
       *
       * @note mca.start() is just a shortcut of the following code.
       * @code
       ...
       * a root node \c s.
       *
       * @note mca.run(s) is just a shortcut of the following code.
       * @code
       * mca.init();
       * mca.addSource(s);
       * mca.start();
      ...
      Value arborescenceCost() const {
        Value sum = 0;
        for (ArcIt it(*_digraph); it != INVALID; ++it) {
          if (arborescence(it)) { sum += (*_cost)[it]; }
        }
        return sum;
      }
       ...
       */
      bool arborescence(Arc arc) const { return (*_pred)[_digraph->target(arc)] == arc; }
       ...
       */
      int dualSize(int k) const { return _dual_variables[k].end - _dual_variables[k].begin; }
      ...
      /**
       * @brief NT iterator for getting a dual variable.
       *
       * This class provides a common style NT iterator for getting a
      ...
      class DualIt {
         ...
         */
        DualIt(const MinCostArborescence& algorithm, int variable) : _algorithm(&algorithm) {
    ...
    typename CostMap::Value minCostArborescence(const Digraph&         digraph,
      ...
      typename MinCostArborescence< Digraph, CostMap >::template SetArborescenceMap< ArborescenceMap >::Create mca(
         digraph, cost);
...
#endif   // _NT_MIN_COST_ARBORESCENCE_H_
