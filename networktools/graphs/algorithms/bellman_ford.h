 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (bellman_ford.h).
 *
...
#ifndef _NT_BELLMAN_FORD_H_
#define _NT_BELLMAN_FORD_H_
...
namespace nt {
  namespace graphs {
    ...
    struct BellmanFordDefaultTraits {
       ...
       */
      using OperationTraits = BellmanFordDefaultOperationTraits< Value >;
       ...
       */
      using PredMap = typename GR::template NodeMap< typename GR::Arc >;
       ...
       * It must conform to the @ref concepts::WriteMap "WriteMap" concept.
       */
      using DistMap = typename GR::template NodeMap< typename LEN::Value >;
 ...
 */
    ...
    class BellmanFord {
      ...
      using Value = typename TR::LengthMap::Value;
      /** @brief The type of the map that stores the arc lengths. */
      ...
      /**
       * @brief The @ref nt::graphs::BellmanFordDefaultOperationTraits
       * "operation traits class" of the algorithm.
       */
      using OperationTraits = typename TR::OperationTraits;

      /**
       * @brief The @ref nt::graphs::BellmanFordDefaultTraits "traits class"
       * of the algorithm.
       */
      using Traits = TR;
      ...
      using MaskMap = typename Digraph::template NodeMap< bool >;
      MaskMap* _mask;
      ...
      void create_maps() {
        ...
        if (!_mask) { _mask = new MaskMap(*_gr); }
      }

      public:
      using Create = BellmanFord;
       ...
       */
      template < class T >
      struct SetPredMap: public BellmanFord< Digraph, LengthMap, SetPredMapTraits< T > > {
        using Create = BellmanFord< Digraph, LengthMap, SetPredMapTraits< T > >;
      };
       ...
       */
      template < class T >
      struct SetDistMap: public BellmanFord< Digraph, LengthMap, SetDistMapTraits< T > > {
        using Create = BellmanFord< Digraph, LengthMap, SetDistMapTraits< T > >;
      };
       ...
       */
      template < class T >
      struct SetOperationTraits: public BellmanFord< Digraph, LengthMap, SetOperationTraitsTraits< T > > {
        using Create = BellmanFord< Digraph, LengthMap, SetOperationTraitsTraits< T > >;
      };
      ...
      bool processNextWeakRound() {
        ...
        for (int i = 0; i < int(_process.size()); ++i) {
          for (OutArcIt it(*_gr, _process[i]); it != INVALID; ++it) {
            Node  target = _gr->target(it);
            Value relaxed = OperationTraits::plus((*_dist)[_process[i]], (*_length)[it]);
            if (OperationTraits::less(relaxed, (*_dist)[target])) {
              _pred->set(target, it);
              _dist->set(target, relaxed);
              if (!(*_mask)[target]) {
       ...
       */
      void start() {
        int num = nt::graphs::countNodes(*_gr) - 1;
        for (int i = 0; i < num; ++i) {
       ...
       */
      bool checkedStart() {
        int num = nt::graphs::countNodes(*_gr);
        for (int i = 0; i < num; ++i) {
       ...
       * - the distance of each node from the root(s).
       *
       * @note bf.run(s) is just a shortcut of the following code.
       * @code
       * bf.init();
       * bf.addSource(s);
       * bf.start();
       ...
       * and build the path manually.
       *
       * @note bf.run(s, num) is just a shortcut of the following code.
       * @code
       * bf.init();
       * bf.addSource(s);
       * bf.limitedStart(num);
      ...
      class ActiveIt {
         ...
         * instance.
         */
        ActiveIt(const BellmanFord& algorithm) : _algorithm(&algorithm) { _index = _algorithm->_process.size() - 1; }
       ...
       */
      nt::graphs::Path< Digraph > negativeCycle() const {
        typename Digraph::template NodeMap< int > state(*_gr, -1);
        nt::graphs::Path< Digraph >               cycle;
        for (int i = 0; i < int(_process.size()); ++i) {
    ...
    struct BellmanFordWizardDefaultTraits {
       ...
       * It must conform to the @ref concepts::WriteMap "WriteMap" concept.
       */
      using DistMap = typename GR::template NodeMap< Value >;
    ...
    class BellmanFordWizardBase: public BellmanFordWizardDefaultTraits< GR, LEN > {
      using Base = BellmanFordWizardDefaultTraits< GR, LEN >;
       ...
       */
      BellmanFordWizardBase() : _graph(0), _length(0), _pred(0), _dist(0), _path(0), _di(0) {}
    ...
    class BellmanFordWizard: public TR {
      ...
      using NodeIt = typename Digraph::NodeIt;
      using Arc = typename Digraph::Arc;
      using ArcIt = typename Digraph::OutArcIt;
       ...
       */
      void run(Node s) {
        BellmanFord< Digraph, LengthMap, TR > bf(*reinterpret_cast< const Digraph* >(Base::_graph),
