 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (fractional_matching.h).
 *
...
#ifndef _NT_FRACTIONAL_MATCHING_H_
#define _NT_FRACTIONAL_MATCHING_H_
...
namespace nt {
  namespace graphs {
    ...
    struct MaxFractionalMatchingDefaultTraits {
       ...
       */
      using MatchingMap = typename Graph::template NodeMap< typename GR::Arc >;
       ...
       * the matching map.
       */
      static MatchingMap* createMatchingMap(const Graph& graph) { return new MatchingMap(graph); }
       ...
       * \sa LinkedElevator
       */
      using Elevator = LinkedElevator< Graph, typename Graph::Node >;
       ...
       */
      static Elevator* createElevator(const Graph& graph, int max_level) { return new Elevator(graph, max_level); }
    };
 ...
 */
    ...
    class MaxFractionalMatching {
      public:
      /**
       * @brief The @ref nt::MaxFractionalMatchingDefaultTraits
       * "traits class" of the algorithm.
       */
      using Traits = TR;
      /** The type of the graph the algorithm runs on. */
      using Graph = typename TR::Graph;
      /** The type of the matching map. */
      using MatchingMap = typename TR::MatchingMap;
      /** The type of the elevator. */
      using Elevator = typename TR::Elevator;
      ...
      using InDegMap = typename Graph::template NodeMap< int >;
      InDegMap* _indeg;
      ...
      void createStructures() {
        ...
        if (!_indeg) { _indeg = new InDegMap(_graph); }
      }

      void destroyStructures() {
        if (_local_matching) { delete _matching; }
        if (_local_level) { delete _level; }
        if (_indeg) { delete _indeg; }
      }
      ...
      public:
      using Create = MaxFractionalMatching;
      ...
      struct SetMatchingMapTraits: public Traits {
        ...
        static MatchingMap* createMatchingMap(const Graph&) {
          NT_ASSERT(false, "MatchingMap is not initialized");
       ...
       */
      template < typename T >
      struct SetMatchingMap: public MaxFractionalMatching< Graph, SetMatchingMapTraits< T > > {
        using Create = MaxFractionalMatching< Graph, SetMatchingMapTraits< T > >;
      };
      ...
      struct SetElevatorTraits: public Traits {
        ...
        static Elevator* createElevator(const Graph&, int) {
          NT_ASSERT(false, "Elevator is not initialized");
          return 0;   // ignore warnings
        }
      };
       ...
       */
      template < typename T >
      struct SetElevator: public MaxFractionalMatching< Graph, SetElevatorTraits< T > > {
        using Create = MaxFractionalMatching< Graph, SetElevatorTraits< T > >;
      };
       ...
       */
      template < typename T >
      struct SetStandardElevator: public MaxFractionalMatching< Graph, SetStandardElevatorTraits< T > > {
        using Create = MaxFractionalMatching< Graph, SetStandardElevatorTraits< T > >;
      };
      ...
      MaxFractionalMatching(const Graph& graph, bool allow_loops = true) :
          _graph(graph), _allow_loops(allow_loops), _local_matching(false), _matching(0), _local_level(false),
          _level(0), _indeg(0) {}

      ~MaxFractionalMatching() noexcept { destroyStructures(); }
      ...
      void init() {
        ...
        for (NodeIt n(_graph); n != INVALID; ++n) {
          if ((*_indeg)[n] == 0) { _level->activate(n); }
        }
      }
      ...
      void start(bool postprocess = true) {
        ...
        while ((n = _level->highestActive()) != INVALID) {
          ...
          for (InArcIt a(_graph, n); a != INVALID; ++a) {
            ...
            if ((*_level)[v] < level) {
              _indeg->set(v, (*_indeg)[v] - 1);
              if ((*_indeg)[v] == 0) { _level->activate(v); }
              _matching->set(u, a);
          ...
          if (_level->emptyLevel(level)) { _level->liftToTop(level); }
        no_more_push:;
        }
        for (NodeIt n(_graph); n != INVALID; ++n) {
      ...
      int matching(const Edge& edge) const {
        return (edge == (*_matching)[_graph.u(edge)] ? 1 : 0) + (edge == (*_matching)[_graph.v(edge)] ? 1 : 0);
      }
       ...
       */
      bool barrier(const Node& node) const { return (*_level)[node] >= _empty_level; }
 ...
 */
    ...
    class MaxWeightedFractionalMatching {
      ...
      void matchedToEven(Node node, int tree) {
        ...
        if (_delta2->state(node) == _delta2->IN_HEAP) { _delta2->erase(node); }

        for (InArcIt a(_graph, node); a != INVALID; ++a) {
          Node  v = _graph.source(a);
          Value rw = (*_node_potential)[node] + (*_node_potential)[v] - dualScale * _weight[a];
          if (node == v) {
            if (_allow_loops && _graph.direction(a)) { _delta3->push(a, rw / 2); }
      ...
      void evenToMatched(Node node, int tree) {
        ...
        for (InArcIt a(_graph, node); a != INVALID; ++a) {
          ...
          if (node == v) {
            if (_allow_loops && _graph.direction(a)) { _delta3->erase(a); }
          } else if ((*_status)[v] == EVEN) {
            _delta3->erase(a);
            if (minrw > rw) {
            ...
            if ((*_pred)[v] == a) {
              ...
              for (OutArcIt aa(_graph, v); aa != INVALID; ++aa) {
                ...
                if ((*_status)[va] != EVEN || _tree_set->find(va) == tree) continue;
                Value rwa = (*_node_potential)[v] + (*_node_potential)[va] - dualScale * _weight[aa];
                if (minrwa > rwa) {
                  minrwa = rwa;
                  mina = aa;
                }
      ...
      void cycleOnEdge(const Edge& edge, int tree) {
        Node                            nca = INVALID;
        nt::TrivialDynamicArray< Node > left_path, right_path;
      ...
      MaxWeightedFractionalMatching(const Graph& graph, const WeightMap& weight, bool allow_loops = true) :
          _graph(graph), _weight(weight), _matching(0), _node_potential(0), _node_num(0), _allow_loops(allow_loops),
          _status(0), _pred(0), _tree_set_index(0), _tree_set(0),

          _delta1_index(0), _delta1(0), _delta2_index(0), _delta2(0), _delta3_index(0), _delta3(0),

          _delta_sum() {}

      ~MaxWeightedFractionalMatching() noexcept { destroyStructures(); }
      ...
      void init() {
        ...
        for (EdgeIt e(_graph); e != INVALID; ++e) {
          ...
          _delta3->push(e, ((*_node_potential)[left] + (*_node_potential)[right] - dualScale * _weight[e]) / 2);
        }
      }
       ...
       *
       * @note mwfm.run() is just a shortcut of the following code.
       * @code
       * mwfm.init();
       * mwfm.start();
       * @endcode
 ...
 */
    ...
    class MaxWeightedPerfectFractionalMatching {
      ...
      ~MaxWeightedPerfectFractionalMatching() noexcept { destroyStructures(); }
