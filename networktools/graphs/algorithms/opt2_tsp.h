 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (opt2_tsp.h).
 *
...
#ifndef _NT_OPT2_TSP_H_
#define _NT_OPT2_TSP_H_
...
namespace nt {
  namespace graphs {
    ...
    class Opt2Tsp {
      ...
      const CostMap&                  _cost;
      Cost                            _sum;
      nt::TrivialDynamicArray< int >  _plist;
      ...
      Cost run(const Path& tour) {
        ...
        int                  first = _gr.id(_gr.source(it)), prev = first, curr = _gr.id(_gr.target(it)), next = -1;
       ...
       * @return The total cost of the found tour.
       */
      Cost run(const nt::TrivialDynamicArray< Node >& tour) {
        _path.clear();
        ...
        _plist.resize(2 * _gr.nodeNum());
        typename nt::TrivialDynamicArray< Node >::const_iterator it = tour.begin();
      ...
      class PathListIt {
        public:
        PathListIt(const nt::TrivialDynamicArray< int >& pl, int i = 0) : plist(&pl), act(i), last(pl[2 * act]) {}
        PathListIt(const nt::TrivialDynamicArray< int >& pl, int i, int l) : plist(&pl), act(i), last(l) {}

        int nextIndex() const { return (*plist)[2 * act] == last ? 2 * act + 1 : 2 * act; }

        int prevIndex() const { return (*plist)[2 * act] == last ? 2 * act : 2 * act + 1; }
        ...
        private:
        const nt::TrivialDynamicArray< int >* plist;
        int                                   act;
        int                                   last;
      };
      ...
      bool checkOpt2(const PathListIt& i, const PathListIt& j) {
        Node u = _gr.nodeFromId(i), un = _gr.nodeFromId(i.next()), v = _gr.nodeFromId(j), vn = _gr.nodeFromId(j.next());

        if (_cost[_gr.edge(u, un)] + _cost[_gr.edge(v, vn)] > _cost[_gr.edge(u, v)] + _cost[_gr.edge(un, vn)]) {
      ...
      restart_search:
        for (PathListIt i(_plist); true; ++i) {
          ...
          if (++j == 0 || ++j == 0) break;
          for (; j != 0 && j != i.prev(); ++j) {
            if (checkOpt2(i, j)) goto restart_search;
          }
        }
