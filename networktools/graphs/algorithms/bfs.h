 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (bfs.h).
 *
 ...
 *   - BfsDistance and BfsDistanceVisit classes added
 */

/**
 * @file
 * @brief Compute the shortest path distances in terms of number of edges.
 *
...
namespace nt {
  namespace graphs {
    ...
    struct BfsDistance {
      ...
      void run(const Digraph& graph, Node s, DistMap&& dist) {
        using Dist = typename DistMap::Value;
        static_assert(std::is_unsigned< Dist >::value, "BfsDistance: Provided DistMap must store unsigned values.");
        clear();
        ...
        while (!_queue.empty()) {
          ...
          _queue.pop();
          const Dist d = dist[v] + 1;
          for (OutArcIt e(graph, v); e != INVALID; ++e) {
            const Node u = graph.target(e);
            Dist&      dist_u = dist[u];
            if (d < dist_u) {
              dist_u = d;
              _queue.push(u);
     ...
     *
     * This class defines the interface of the BfsDistanceVisit events, and it could be the base of a real
     * visitor class.
     *
     * @tparam GR
     */
    ...
    struct DefaultBfsDistanceVisitor {
      using Digraph = GR;
     ...
     * @tparam GR
     */
    template < typename GR, typename VS = DefaultBfsDistanceVisitor< GR > >
    struct BfsDistanceVisit {
      using Digraph = GR;
      TEMPLATE_DIGRAPH_TYPEDEFS(Digraph);

      using Dist = uint32_t;
      using DistMap = typename Digraph::template StaticNodeMap< Dist >;
      using Visitor = VS;

      constexpr static Dist MAX = ~static_cast< Dist >(0);
      ...
      DefaultBfsDistanceVisitor< GR > _default_visitor;
      Visitor&                        _visitor;

      BfsDistanceVisit(const Digraph& g) : _g(g), _dist_map(g), _visitor(_default_visitor) {}
      BfsDistanceVisit(const Digraph& g, Visitor& visitor) : _g(g), _dist_map(g), _visitor(visitor) {}

      void init() noexcept { clear(); }

      void addSource(Node s) noexcept {
        _dist_map[s] = 0;
        _queue.push(s);
        _visitor.start(s);
        _visitor.reach(s);
      ...
      Node processNextNode() noexcept {
        ...
        const Dist d = _dist_map[v] + 1;
        for (OutArcIt e(_g, v); e != INVALID; ++e) {
          const Node u = _g.target(e);
          Dist&      dist_u = _dist_map[u];
          if (d < dist_u) {
            _visitor.discover(e);
            _visitor.reach(u);
            dist_u = d;
      ...
      Node nextNode() const noexcept { return _queue.empty() ? INVALID : _queue.front(); }
      bool emptyQueue() const noexcept { return _queue.empty(); }
      int  queueSize() const noexcept { return _queue.size(); }
      ...
      int dist(Node v) const noexcept { return _dist_map[v]; }

      /**
       * @brief Returns \c true if \c v is reached from the root(s).
       *
       *\pre Either \ref run(Node) "run()" or \ref init()
       * must be called before using this function.
       */
      bool reached(Node v) const { return _dist_map[v] < MAX; }
       ...
       */
      const Visitor& visitor() const noexcept { return _visitor; }
      Visitor&       visitor() noexcept { return _visitor; }
 ...
 */
...
#else
    template < typename GR, typename TR = BfsDefaultTraits< GR > >
#endif
    class Bfs {
      public:
      /** The type of the digraph the algorithm runs on. */
      ...
      /**
       * The @ref nt::graphs::BfsDefaultTraits "traits class" of the algorithm.
       */
      using Traits = TR;
      ...
      nt::TrivialDynamicArray< typename Digraph::Node > _queue;
      ...
      struct SetPredMap: public Bfs< Digraph, SetPredMapTraits< T > > {
        using Create = Bfs< Digraph, SetPredMapTraits< T > >;
      };
      ...
      struct SetDistMap: public Bfs< Digraph, SetDistMapTraits< T > > {
        using Create = Bfs< Digraph, SetDistMapTraits< T > >;
      };
      ...
      struct SetReachedMap: public Bfs< Digraph, SetReachedMapTraits< T > > {
        using Create = Bfs< Digraph, SetReachedMapTraits< T > >;
      };
      ...
      struct SetProcessedMap: public Bfs< Digraph, SetProcessedMapTraits< T > > {
        using Create = Bfs< Digraph, SetProcessedMapTraits< T > >;
      };
       ...
       */
      struct SetStandardProcessedMap: public Bfs< Digraph, SetStandardProcessedMapTraits > {
        using Create = Bfs< Digraph, SetStandardProcessedMapTraits >;
      };
      ...
      void init() {
        create_maps();
        _queue.resize(nt::graphs::countNodes(*G));
       ...
       * is empty.
       */
      Node nextNode() const { return _queue_tail < _queue_head ? _queue[_queue_tail] : INVALID; }
       ...
       *
       * @note <tt>b.start()</tt> is just a shortcut of the following code.
       * @code
       * while ( !b.emptyQueue() ) {
       * b.processNextNode();
       * }
       ...
       * added with addSource() before using this function.
       *
       * @note <tt>b.start(t)</tt> is just a shortcut of the following code.
       * @code
       * bool reach = false;
       ...
       * added with addSource() before using this function.
       *
       * @note <tt>b.start(nm)</tt> is just a shortcut of the following code.
       * @code
       * Node rnode = INVALID;
       ...
       *
       * @note <tt>b.run(s)</tt> is just a shortcut of the following code.
       * @code
       * b.init();
       * b.addSource(s);
       * b.start();
       ...
       *
       * @note Apart from the return value, <tt>b.run(s,t)</tt> is just a
    ...
    class BfsWizardBase: public BfsWizardDefaultTraits< GR > {
      using Base = BfsWizardDefaultTraits< GR >;
       ...
       */
      BfsWizardBase() : _g(0), _reached(0), _processed(0), _pred(0), _dist(0), _path(0), _di(0) {}
    ...
    struct BfsVisitDefaultTraits {
       ...
       * we would like to define the ReachedMap.
       */
      static ReachedMap* createReachedMap(const Digraph& digraph) { return new ReachedMap(digraph); }
    };
...
#else
    template < typename GR, typename VS = BfsVisitor< GR >, typename TR = BfsVisitDefaultTraits< GR > >
#endif
    class BfsVisit {
      public:
      /** The traits class. */
      ...
      /** The type of the map that indicates which nodes are reached. */
      using ReachedMap = typename Traits::ReachedMap;
      ...
      nt::TrivialDynamicArray< typename Digraph::Node > _list;
       ...
       */
      template < class T >
      struct SetReachedMap: public BfsVisit< Digraph, Visitor, SetReachedMapTraits< T > > {
        using Create = BfsVisit< Digraph, Visitor, SetReachedMapTraits< T > >;
      };
      /** @} */
      ...
      BfsVisit(const Digraph& digraph, Visitor& visitor) :
          _digraph(&digraph), _visitor(&visitor), _reached(0), local_reached(false) {}
      ...
      void init() {
        create_maps();
        _list.resize(nt::graphs::countNodes(*_digraph));
       ...
       * is empty.
       */
      Node nextNode() const { return _list_front != _list_back ? _list[_list_front + 1] : INVALID; }
