 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (insertion_tsp.h).
 *
...
#ifndef _NT_INSERTION_TSP_H_
#define _NT_INSERTION_TSP_H_
...
namespace nt {
  namespace graphs {
    ...
    class InsertionTsp {
      ...
      const FullGraph&                _gr;
      const CostMap&                  _cost;
      nt::TrivialDynamicArray< Node > _notused;
      nt::TrivialDynamicArray< Node > _tour;
      Cost                            _sum;
       ...
       */
      InsertionTsp(const FullGraph& gr, const CostMap& cost) : _gr(gr), _cost(cost) {}
       ...
       * @pre run() must be called before using this function.
       */
      const nt::TrivialDynamicArray< Node >& tourNodes() const { return _tour; }
      ...
      void tour(Path& path) const {
        ...
        if (int(_tour.size()) >= 2) { path.addBack(_gr.arc(_tour.back(), _tour.front())); }
      }
      ...
      void init(bool min) {
        ...
        for (NodeIt n(_gr); n != INVALID; ++n) {
          if (n != _gr.u(min_edge) && n != _gr.v(min_edge)) { _notused.push_back(n); }
        }
      ...
      class ComparingSelection {
        public:
        ComparingSelection(const FullGraph&                 gr,
                           const CostMap&                   cost,
                           nt::TrivialDynamicArray< Node >& tour,
                           nt::TrivialDynamicArray< Node >& notused) :
            _gr(gr),
            _cost(cost), _tour(tour), _notused(notused), _dist(gr, 0), _compare() {
      ...
      class CheapestSelection {
        ...
        Cost costDiff(Node u, Node v, Node w) const {
          return _cost[_gr.edge(u, w)] + _cost[_gr.edge(v, w)] - _cost[_gr.edge(u, v)];
        }

        public:
        CheapestSelection(const FullGraph&                 gr,
                          const CostMap&                   cost,
                          nt::TrivialDynamicArray< Node >& tour,
                          nt::TrivialDynamicArray< Node >& notused) :
            _gr(gr),
            _cost(cost), _tour(tour), _notused(notused), _ins_cost(gr, 0), _ins_pos(gr, -1) {
      ...
      class RandomSelection {
        public:
        RandomSelection(const FullGraph&,
                        const CostMap&,
                        nt::TrivialDynamicArray< Node >&,
                        nt::TrivialDynamicArray< Node >& notused) :
            _notused(notused) {}

        Node select(const nt::Random& rnd = nt::Random()) const {
      ...
      class DefaultInsertion {
        ...
        public:
        DefaultInsertion(const FullGraph&                 gr,
                         const CostMap&                   cost,
                         nt::TrivialDynamicArray< Node >& tour,
                         Cost&                            total_cost) :
            _gr(gr),
            _cost(cost), _tour(tour), _total(total_cost) {}
      ...
      class CheapestInsertion {
        ...
        public:
        CheapestInsertion(const FullGraph&, const CostMap&, nt::TrivialDynamicArray< Node >&, Cost& total_cost) :
            _total(total_cost) {}
