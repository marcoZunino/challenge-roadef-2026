 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (preflow.h).
 *
...
#ifndef _NT_PREFLOW_H_
#define _NT_PREFLOW_H_
 ...
 */
    ...
    class Preflow {
      public:
      /**
       * The @ref nt::graphs::PreflowDefaultTraits "traits class" of the algorithm.
       */
      using Traits = TR;
      ...
      void createStructures() {
        ...
        if (!_excess) { _excess = new ExcessMap(_graph); }
      }
       ...
       */
      template < typename T >
      struct SetFlowMap: public Preflow< Digraph, CapacityMap, SetFlowMapTraits< T > > {
        using Create = Preflow< Digraph, CapacityMap, SetFlowMapTraits< T > >;
      };
       ...
       */
      template < typename T >
      struct SetElevator: public Preflow< Digraph, CapacityMap, SetElevatorTraits< T > > {
        using Create = Preflow< Digraph, CapacityMap, SetElevatorTraits< T > >;
      };
       ...
       */
      template < typename T >
      struct SetStandardElevator: public Preflow< Digraph, CapacityMap, SetStandardElevatorTraits< T > > {
        using Create = Preflow< Digraph, CapacityMap, SetStandardElevatorTraits< T > >;
      };
      ...
      Preflow(const Digraph& digraph, const CapacityMap& capacity, Node source, Node target) :
          _graph(digraph), _capacity(&capacity), _node_num(0), _source(source), _target(target), _flow(0),
          _local_flow(false), _level(0), _local_level(false), _excess(0), _tolerance(), _phase() {}
       ...
       * Destructor.
       */
      ~Preflow() noexcept { destroyStructures(); }
      ...
      void init() {
        ...
        while (!queue.empty()) {
          _level->initNewLevel();
          nt::TrivialDynamicArray< Node > nqueue;
        ...
        for (OutArcIt e(_graph, _source); e != INVALID; ++e) {
          if (_tolerance.positive((*_capacity)[e])) {
            ...
            if (u != _target && !_level->active(u)) { _level->activate(u); }
          }
        }
      }
      ...
      bool init(const FlowMap& flowMap) {
        ...
        while (!queue.empty()) {
          ...
          for (int i = 0; i < int(queue.size()); ++i) {
            ...
            for (InArcIt e(_graph, n); e != INVALID; ++e) {
              Node u = _graph.source(e);
              if (!reached[u] && _tolerance.positive((*_capacity)[e] - (*_flow)[e])) {
                reached[u] = true;
                _level->initAddItem(u);
                nqueue.push_back(u);
              }
        ...
        for (NodeIt n(_graph); n != INVALID; ++n)
          if (n != _source && n != _target && _tolerance.positive((*_excess)[n])) _level->activate(n);
        ...
        while (true) {
          ...
          while (num > 0) {
            ...
            for (OutArcIt e(_graph, n); e != INVALID; ++e) {
              ...
              Node v = _graph.target(e);
              if ((*_level)[v] < level) {
                if (!_level->active(v) && v != _target) { _level->activate(v); }
      ...
      void startSecondPhase() {
        ...
        while ((n = _level->highestActive()) != INVALID) {
          ...
          for (OutArcIt e(_graph, n); e != INVALID; ++e) {
            ...
            Node v = _graph.target(e);
            if ((*_level)[v] < level) {
              if (!_level->active(v) && v != _source) { _level->activate(v); }
              if (!_tolerance.less(rem, excess)) {
                _flow->set(e, (*_flow)[e] + excess);
                (*_excess)[v] += excess;
                excess = 0;
       ...
       * @note pf.run() is just a shortcut of the following code.
       * @code
       * pf.init();
       * pf.startFirstPhase();
       * pf.startSecondPhase();
       ...
       * @note pf.runMinCut() is just a shortcut of the following code.
       * @code
       * pf.init();
       * pf.startFirstPhase();
       * @endcode
       ...
       * using this function.
       */
      bool minCut(const Node& node) const { return ((*_level)[node] == _level->maxLevel()) == _phase; }
