 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (euler.h).
 *
 ...
 * @file
 * @brief Euler tour iterators and a function for checking the \e Eulerian property.
 *
...
namespace nt {
  namespace graphs {
    ...
    class DiEulerIt {
      ...
      using OutArcIt = typename GR::OutArcIt;
      using InArcIt = typename GR::InArcIt;
      ...
      DiEulerIt& operator++() {
        ...
        euler.pop_front();
        typename nt::List< Arc >::iterator next = euler.begin();
    ...
    class EulerIt {
       ...
       */
      EulerIt(const GR& gr, typename GR::Node start = INVALID) : g(gr), narc(g), visited(g, false) {
        if (start == INVALID) {
          NodeIt n(g);
          while (n != INVALID && OutArcIt(g, n) == INVALID)
            ++n;
     ...
     * for each node.
     *
     * @note There are (di)graphs that are not Eulerian, but still have an
    ...
    bool
#else
    typename std::enable_if< nt::concepts::UndirectedTagIndicator< GR >, bool >::type eulerian(const GR& g) {
      for (typename GR::NodeIt n(g); n != INVALID; ++n)
        if (countIncEdges(g, n) % 2) return false;
      return connected(g);
    }
    template < class GR >
    typename std::enable_if< !nt::concepts::UndirectedTagIndicator< GR >, bool >::type
#endif
       eulerian(const GR& g) {
      for (typename GR::NodeIt n(g); n != INVALID; ++n)
