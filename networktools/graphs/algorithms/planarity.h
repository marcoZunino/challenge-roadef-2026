 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (planarity.h).
 *
 ...
 *   - Replaced std::list with nt::List
...
#ifndef _NT_PLANARITY_H_
#define _NT_PLANARITY_H_
...
namespace nt {
  namespace graphs {
    ...
    namespace _planarity_bits {
      ...
      struct PlanarityVisitor: DefaultDfsVisitor< Graph > {
        TEMPLATE_GRAPH_TYPEDEFS(Graph);

        using PredMap = typename Graph::template NodeMap< Arc >;

        using TreeMap = typename Graph::template EdgeMap< bool >;

        using OrderMap = typename Graph::template NodeMap< int >;
        using OrderList = nt::TrivialDynamicArray< Node >;

        using LowMap = typename Graph::template NodeMap< int >;
        using AncestorMap = typename Graph::template NodeMap< int >;
        ...
        PlanarityVisitor(const Graph& graph,
                         ...
                         LowMap&      low_map) :
            _graph(graph),
            _pred_map(pred_map), _tree_map(tree_map), _order_map(order_map), _order_list(order_list),
        ...
        void examine(const Arc& arc) {
          ...
          if (_order_map[target] < _order_map[source] && !_tree_map[arc]) {
            if (_low_map[source] > _order_map[target]) { _low_map[source] = _order_map[target]; }
            if (_ancestor_map[source] > _order_map[target]) { _ancestor_map[source] = _order_map[target]; }
          }
        }
        ...
        void backtrack(const Arc& arc) {
          ...
          if (_low_map[source] > _low_map[target]) { _low_map[source] = _low_map[target]; }
        }
      ...
      class PlanarityChecking {
        ...
        using NodeDataNode = _planarity_bits::NodeDataNode< Graph >;
        using NodeData = nt::TrivialDynamicArray< NodeDataNode >;

        using ChildListNode = _planarity_bits::ChildListNode< Graph >;
        using ChildLists = typename Graph::template NodeMap< ChildListNode >;

        using MergeRoots = typename Graph::template NodeMap< nt::List< int > >;

        using EmbedArc = typename Graph::template NodeMap< bool >;

        public:
        PlanarityChecking(const Graph& graph) : _graph(graph) {}

        bool run() {
          using Visitor = _planarity_bits::PlanarityVisitor< Graph >;
          ...
          Visitor                    visitor(_graph, pred_map, tree_map, order_map, order_list, ancestor_map, low_map);
          ...
          for (int i = order_list.size() - 1; i >= 0; --i) {
            ...
            for (OutArcIt e(_graph, node); e != INVALID; ++e) {
              ...
              if (order_map[source] < order_map[target] && !tree_map[e]) {
                ...
                walkUp(target, source, i, pred_map, low_map, order_map, order_list, node_data, merge_roots);
              }
            }

            for (typename MergeRoots::Value::iterator it = merge_roots[node].begin(); it != merge_roots[node].end();
                 ++it) {
              int rn = *it;
              walkDown(rn, i, node_data, order_list, child_lists, ancestor_map, low_map, embed_arc, merge_roots);
            }
            ...
            for (OutArcIt e(_graph, node); e != INVALID; ++e) {
              ...
              if (order_map[source] < order_map[target] && !tree_map[e]) {
                if (embed_arc[target]) { return false; }
              }
            }
          }
        ...
        private:
        void createChildLists(const TreeMap&  tree_map,
          ...
          for (NodeIt n(_graph); n != INVALID; ++n) {
            ...
            nt::TrivialDynamicArray< Node > targets;
            for (OutArcIt e(_graph, n); e != INVALID; ++e) {
              Node target = _graph.target(e);

              if (order_map[source] < order_map[target] && tree_map[e]) { targets.push_back(target); }
            }
        ...
        void walkDown(int          rn,
                      ...
                      EmbedArc&    embed_arc,
                      MergeRoots&  merge_roots) {
          nt::TrivialDynamicArray< nt::Pair< int, bool > > merge_stack;
          ...
          for (int di = 0; di < 2; ++di) {
            ...
            while (n != rn) {
              ...
              if (embed_arc[node]) {
                ...
                while (!merge_stack.empty()) {
                  ...
                  if (child_lists[child].prev != INVALID) {
                    child_lists[child_lists[child].prev].next = child_lists[child].next;
                  } else {
                    child_lists[parent].first = child_lists[child].next;
                  }

                  if (child_lists[child].next != INVALID) {
                    child_lists[child_lists[child].next].prev = child_lists[child].prev;
                  }
                ...
                if (node_data[n].prev == node_data[n].next && node_data[n].inverted) { d = !d; }
              ...
              if (!merge_roots[node].empty()) {
                ...
                if (!external(xnode, rorder, child_lists, ancestor_map, low_map)) {
                  rd = true;
                } else if (!external(ynode, rorder, child_lists, ancestor_map, low_map)) {
                  rd = false;
                ...
                merge_stack.push_back({rn, rd});

                pn = rn;
                n = rd ? xn : yn;

              } else if (!external(node, rorder, child_lists, ancestor_map, low_map)) {
                int nn = (node_data[n].next != pn ? node_data[n].next : node_data[n].prev);
                ...
                node_data[nn].inverted = (node_data[nn].prev == node_data[nn].next && nd != rd);
            ...
            if (!merge_stack.empty() || n == rn) { break; }
          }
        }

        void initFace(const Node& node, NodeData& node_data, const OrderMap& order_map, const OrderList& order_list) {
        ...
        bool
           external(const Node& node, int rorder, ChildLists& child_lists, AncestorMap& ancestor_map, LowMap& low_map) {
        ...
        bool pertinent(const Node& node, const EmbedArc& embed_arc, const MergeRoots& merge_roots) {
    ...
    class PlanarEmbedding {
      ...
      using EmbedArc = typename Graph::template NodeMap< Arc >;

      using ArcListNode = _planarity_bits::ArcListNode< Graph >;
      using ArcLists = typename Graph::template ArcMap< ArcListNode >;

      using FlipMap = typename Graph::template NodeMap< bool >;

      using TypeMap = typename Graph::template NodeMap< int >;

      enum IsolatorNodeType { HIGHX = 6, LOWX = 7, HIGHY = 8, LOWY = 9, ROOT = 10, PERTINENT = 11, INTERNAL = 12 };
       ...
       * @see embeddingMap()
       */
      using EmbeddingMap = typename Graph::template ArcMap< Arc >;
       ...
       */
      PlanarEmbedding(const Graph& graph) : _graph(graph), _embedding(_graph), _kuratowski(graph, false) {}
      ...
      bool run(bool kuratowski = true) {
        ...
        for (int i = order_list.size() - 1; i >= 0; --i) {
          ...
          for (OutArcIt e(_graph, node); e != INVALID; ++e) {
            ...
            if (order_map[source] < order_map[target] && tree_map[e]) {
              initFace(target, arc_lists, node_data, pred_map, order_map, order_list);
            }
          }
        ...
        for (int i = 0; i < int(order_list.size()); ++i) {
          mergeRemainingFaces(order_list[i], node_data, order_list, order_map, child_lists, arc_lists);
          storeEmbedding(order_list[i], node_data, order_map, pred_map, arc_lists, flip_map);
        }
            ...
            if (embed_arc[node] != INVALID) {
              ...
              while (!merge_stack.empty()) {
                ...
                // Merging external faces
                {
                  ...
                  if (node_data[cn].prev == node_data[cn].next && node_data[cn].inverted) { cd = !cd; }
                }
      ...
      void mergeRemainingFaces(const Node& node,
                               NodeData&   node_data,
                               OrderList&  order_list,
                               OrderMap&   order_map,
                               ChildLists& child_lists,
      ...
      void storeEmbedding(const Node& node,
                          NodeData&   node_data,
                          OrderMap&   order_map,
                          PredMap&    pred_map,
                          ArcLists&   arc_lists,
        ...
        while (arc != first) {
          Arc next = arc_lists[arc].prev == prev ? arc_lists[arc].next : arc_lists[arc].prev;
          prev = arc;
          arc = next;
          _embedding[prev] = arc;
        }
      ...
      bool external(const Node& node, int rorder, ChildLists& child_lists, AncestorMap& ancestor_map, LowMap& low_map) {
        Node child = child_lists[node].first;
      ...
      int lowPoint(
         const Node& node, OrderMap& order_map, ChildLists& child_lists, AncestorMap& ancestor_map, LowMap& low_map) {
        int low_point;
        ...
        if (low_point > ancestor_map[node]) { low_point = ancestor_map[node]; }

        return low_point;
      }

      int findComponentRoot(Node root, Node node, ChildLists& child_lists, OrderMap& order_map, OrderList& order_list) {
        ...
        while (child != INVALID) {
          ...
          if (corder > order && corder < norder) { order = corder; }
      ...
      Node findPertinent(
         Node node, OrderMap& order_map, NodeData& node_data, EmbedArc& embed_arc, MergeRoots& merge_roots) {
        ...
        while (true) {
          ...
          if (pert && ext) {
            ...
            while (!external(cnode, rorder, child_lists, ancestor_map, low_map)) {
              ...
              if (_graph.target(arc) == pred) { arc = arc_lists[arc].next; }
              _kuratowski.set(arc, true);
      ...
      void orientComponent(Node      root,
                           int       rn,
                           OrderMap& order_map,
                           PredMap&  pred_map,
                           NodeData& node_data,
        ...
        nt::TrivialDynamicArray< Node > st, qu;
        ...
        while (!st.empty()) {
          ...
          while (arc != last) {
            ...
            Arc next = arc_lists[arc].next != pred ? arc_lists[arc].next : arc_lists[arc].prev;
            pred = arc;
            arc = next;
          }
        }
        ...
        for (int i = 0; i < int(qu.size()); ++i) {
          ...
          arc = arc_lists[arc].next;
          while (arc != last) {
            if (arc_lists[arc].next == pred) { std::swap(arc_lists[arc].next, arc_lists[arc].prev); }
            pred = arc;
            arc = arc_lists[arc].next;
          }
        }
      ...
      void setFaceFlags(
         Node root, Node wnode, Node ynode, Node xnode, OrderMap& order_map, NodeData& node_data, TypeMap& type_map) {
      ...
      void findInternalPath(nt::TrivialDynamicArray< Arc >& ipath,
                            Node                            wnode,
                            Node                            root,
                            TypeMap&                        type_map,
                            OrderMap&                       order_map,
        ...
        while (true) {
          Arc arc = st.back();
          if (type_map[_graph.target(arc)] == LOWX || type_map[_graph.target(arc)] == HIGHX) { break; }
        ...
        for (int i = 0; i < int(st.size()); ++i) {
          if (type_map[_graph.target(st[i])] != LOWY && type_map[_graph.target(st[i])] != HIGHY) {
            for (; i < int(st.size()); ++i) {
      ...
      void setInternalFlags(nt::TrivialDynamicArray< Arc >& ipath, TypeMap& type_map) {
        for (int i = 1; i < int(ipath.size()); ++i) {
          type_map[_graph.source(ipath[i])] = INTERNAL;
        }
      }

      void findPilePath(nt::TrivialDynamicArray< Arc >& ppath,
                        Node                            root,
                        TypeMap&                        type_map,
                        OrderMap&                       order_map,
                        NodeData&                       node_data,
        ...
        while (st.size() > 1) {
          Arc arc = st.back();
          if (type_map[_graph.target(arc)] == INTERNAL) { break; }
      ...
      int markExternalPath(Node         node,
                           OrderMap&    order_map,
                           ChildLists&  child_lists,
                           PredMap&     pred_map,
                           AncestorMap& ancestor_map,
      ...
      void markPertinentPath(Node        node,
                             OrderMap&   order_map,
                             NodeData&   node_data,
                             ArcLists&   arc_lists,
                             EmbedArc&   embed_arc,
      ...
      void markFacePath(Node ynode, Node xnode, OrderMap& order_map, NodeData& node_data) {
      ...
      void markInternalPath(nt::TrivialDynamicArray< Arc >& path) {
        for (int i = 0; i < int(path.size()); ++i) {
          _kuratowski.set(path[i], true);
        }
      }

      void markPilePath(nt::TrivialDynamicArray< Arc >& path) {
        for (int i = 0; i < int(path.size()); ++i) {
          _kuratowski.set(path[i], true);
        }
      }

      void isolateKuratowski(Arc          arc,
                             NodeData&    node_data,
                             ArcLists&    arc_lists,
                             FlipMap&     flip_map,
                             OrderMap&    order_map,
        ...
        // Minor-A
        {
          ...
          if (root != _graph.source(arc)) {
            orientComponent(root, rn, order_map, pred_map, node_data, arc_lists, flip_map, type_map);
            markFacePath(root, root, order_map, node_data);
            int xlp = markExternalPath(xnode, order_map, child_lists, pred_map, ancestor_map, low_map);
            int ylp = markExternalPath(ynode, order_map, child_lists, pred_map, ancestor_map, low_map);
            markPredPath(root, order_list[xlp < ylp ? xlp : ylp], pred_map);
            Node lwnode = findPertinent(ynode, order_map, node_data, embed_arc, merge_roots);

            markPertinentPath(lwnode, order_map, node_data, arc_lists, embed_arc, merge_roots);
        ...
        Node wnode = findPertinent(ynode, order_map, node_data, embed_arc, merge_roots);
        ...
        if (!merge_roots[wnode].empty()) {
          ...
          if (low_map[rep] < rorder) {
            ...
            markPertinentPath(lwnode, order_map, node_data, arc_lists, embed_arc, merge_roots);
            int zlp = markExternalPath(lznode, order_map, child_lists, pred_map, ancestor_map, low_map);
        ...
        Node                           pxnode, pynode;
        nt::TrivialDynamicArray< Arc > ipath;
        findInternalPath(ipath, wnode, root, type_map, order_map, node_data, arc_lists);
        ...
        // Minor-C
        {
          if (type_map[_graph.source(ipath.front())] == HIGHY) {
            if (type_map[_graph.target(ipath.back())] == HIGHX) { markFacePath(xnode, pxnode, order_map, node_data); }
            markFacePath(root, xnode, order_map, node_data);
            markPertinentPath(wnode, order_map, node_data, arc_lists, embed_arc, merge_roots);
        ...
        nt::TrivialDynamicArray< Arc > ppath;
        ...
        // Minor-E*
        {
          if (!external(wnode, rorder, child_lists, ancestor_map, low_map)) {
            Node znode = findExternal(pynode, rorder, order_map, child_lists, ancestor_map, low_map, node_data);
            ...
            if (type_map[znode] == LOWY) {
              ...
              markInternalPath(ipath);
              int xlp = markExternalPath(xnode, order_map, child_lists, pred_map, ancestor_map, low_map);
              int zlp = markExternalPath(znode, order_map, child_lists, pred_map, ancestor_map, low_map);
          ...
          int xlp = markExternalPath(xnode, order_map, child_lists, pred_map, ancestor_map, low_map);
          int ylp = markExternalPath(ynode, order_map, child_lists, pred_map, ancestor_map, low_map);
          int wlp = markExternalPath(wnode, order_map, child_lists, pred_map, ancestor_map, low_map);
    ...
    namespace _planarity_bits {
      ...
      void makeConnected(Graph& graph, EmbeddingMap& embedding) {
        DefaultDfsVisitor< Graph >                    null_visitor;
        DfsVisit< Graph, DefaultDfsVisitor< Graph > > dfs(graph, null_visitor);
        dfs.init();
      ...
      void makeBiNodeConnected(Graph& graph, EmbeddingMap& embedding) {
        ...
        nt::TrivialDynamicArray< typename Graph::Arc > arcs;
        ...
        for (int i = 0; i < int(arcs.size()); ++i) {
          ...
          while (e != l) {
            ...
            if (visited[graph.source(e)]) {
              typename Graph::Arc n = graph.direct(graph.addEdge(graph.source(p), graph.target(e)), true);
              embedding[n] = p;
      ...
      void makeMaxPlanar(Graph& graph, EmbeddingMap& embedding) {
        ...
        typename Graph::template ArcMap< bool >        processed(graph, false);
        IterableBoolMap< Graph, typename Graph::Node > visited(graph, false);
        ...
        for (int i = 0; i < int(arcs.size()); ++i) {
          ...
          if (oppe == INVALID) {
            ...
            e = embedding[graph.oppositeArc(e)];
            while (graph.target(e) != s) {
              typename Graph::Arc n = graph.direct(graph.addEdge(s, graph.source(e)), true);
            ...
            while (graph.target(e) != t) {
              typename Graph::Arc n = graph.direct(graph.addEdge(t, graph.source(e)), true);
    ...
    class PlanarDrawing {
      ...
      using Point = dim2::Point< int >;
      /**
       * @brief The map type for storing the coordinates of the nodes
       */
      using PointMap = typename Graph::template NodeMap< Point >;
      ...
      void drawing(const AuxGraph& graph, const AuxEmbeddingMap& next, PointMap& point_map) {
        ...
        conn[anode] = conn[bnode] = -2;
        {
          ...
          for (OutArcIt e(graph, bnode); e != INVALID; ++e) {
            ...
            if (conn[m] == -1) {
              ...
              if (conn[graph.target(next[pe])] == -2) { conn[m] -= 1; }
              if (conn[graph.target(prev[pe])] == -2) { conn[m] -= 1; }
        ...
        nt::TrivialDynamicArray< Node > aorder, border, corder;

        {
          typename AuxGraph::template NodeMap< bool > processed(graph, false);
          nt::TrivialDynamicArray< Node >             st;
          for (NodeIt n(graph); n != INVALID; ++n) {
        ...
        for (int i = aorder.size() - 1; i >= 0; --i) {
          ...
          atree[n] = 1;
          for (OutArcIt e(graph, n); e != INVALID; ++e) {
            if (apred[graph.target(e)] == n) { atree[n] += atree[graph.target(e)]; }
          }
        }
        ...
        for (int i = border.size() - 1; i >= 0; --i) {
          ...
          btree[n] = 1;
          for (OutArcIt e(graph, n); e != INVALID; ++e) {
            if (bpred[graph.target(e)] == n) { btree[n] += btree[graph.target(e)]; }
          }
        }
        ...
        for (NodeIt n(graph); n != INVALID; ++n) {
          point_map[n].x = bpath_atree[n] + cpath_atree[n] - atree[n] - cpath[n] + 1;
          point_map[n].y = cpath_btree[n] + apath_btree[n] - btree[n] - apath[n] + 1;
        }
      }
      ...
      void run(const EmbeddingMap& embedding) {
        using AuxGraph = SmartEdgeGraph< Graph >;

        if (3 * nt::graphs::countNodes(_graph) - 6 == nt::graphs::countEdges(_graph)) {
          drawing(_graph, embedding, _point_map);
          return;
        }

        AuxGraph                                                     aux_graph(_graph);
        typename AuxGraph::template ArcMap< typename AuxGraph::Arc > aux_embedding(aux_graph);

        {
          ...
          for (EdgeIt e(_graph); e != INVALID; ++e) {
            ...
            aux_embedding[aux_graph.direct(ref[e], true)] = aux_graph.direct(ref[ee], _graph.direction(ee));
            ee = embedding[_graph.direct(e, false)];
            aux_embedding[aux_graph.direct(ref[e], false)] = aux_graph.direct(ref[ee], _graph.direction(ee));
          }
        }
    ...
    namespace _planarity_bits {
      ...
      class KempeFilter {
        public:
        using Key = typename ColorMap::Key;
        using Value = bool;
        ...
        Value operator[](const Key& key) const { return _color_map[key] == _first || _color_map[key] == _second; }
    ...
    class PlanarColoring {
       ...
       * @see Palette, Color
       */
      using ColorMap = ComposeMap< Palette, IndexMap >;
       ...
       * contain parallel or loop arcs.
       */
      PlanarColoring(const Graph& graph) : _graph(graph), _color_map(graph), _palette(0) {
       ...
       * five or six distinct @ref nt::Color "colors".
       */
       ...
       * @note This function can return \c true if the graph is not
      ...
      bool runSixColoring() {
        ...
        nt::TrivialDynamicArray< Node > order;
        ...
        while (!heap.empty()) {
          ...
          for (OutArcIt e(_graph, n); e != INVALID; ++e) {
            ...
            if (_color_map[t] == -2) { heap.decrease(t, heap[t] - 1); }
          }
        }

        for (int i = order.size() - 1; i >= 0; --i) {
          nt::TrivialDynamicArray< bool > forbidden(6, false);
          for (OutArcIt e(_graph, order[i]); e != INVALID; ++e) {
            Node t = _graph.runningNode(e);
            if (_color_map[t] != -1) { forbidden[_color_map[t]] = true; }
          }
          for (int k = 0; k < 6; ++k) {
            if (!forbidden[k]) {
          ...
          if (_color_map[order[i]] == -1) { return false; }
        }
        return true;
      }
      ...
      bool recolor(const Node& u, const Node& v) {
        ...
        using KempeFilter = _planarity_bits::KempeFilter< IndexMap >;
        KempeFilter filter(_color_map, ucolor, vcolor);

        using KempeGraph = FilterNodes< const Graph, const KempeFilter >;
        KempeGraph kempe_graph(_graph, filter);

        nt::TrivialDynamicArray< Node > comp;
      ...
      void kempeRecoloring(const Node& node, const EmbeddingMap& embedding) {
        nt::TrivialDynamicArray< Node > nodes;
        nodes.reserve(4);
      ...
      void runFiveColoring(const EmbeddingMap& embedding) {
        ...
        for (int i = order.size() - 1; i >= 0; --i) {
          nt::TrivialDynamicArray< bool > forbidden(5, false);
          for (OutArcIt e(_graph, order[i]); e != INVALID; ++e) {
            Node t = _graph.runningNode(e);
            if (_color_map[t] != -1) { forbidden[_color_map[t]] = true; }
          }
          ...
          if (_color_map[order[i]] == -1) { kempeRecoloring(order[i], embedding); }
        }
      }
