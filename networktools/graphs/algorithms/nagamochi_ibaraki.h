 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (nagamochi_ibaraki.h).
 *
...
#ifndef _NT_NAGAMOCHI_IBARAKI_H_
#define _NT_NAGAMOCHI_IBARAKI_H_
...
namespace nt {
  namespace graphs {
    ...
    struct NagamochiIbarakiDefaultTraits {
...
#endif
      {
        NT_ASSERT(false, "CapacityMap is not initialized");
       ...
       */
      using HeapCrossRef = typename Graph::template NodeMap< int >;
       ...
       * @ref HeapCrossRef.
       */
      static HeapCrossRef* createHeapCrossRef(const Graph& g) { return new HeapCrossRef(g); }
       ...
       * \sa NagamochiIbaraki
       */
      using Heap = BinHeap< Value, HeapCrossRef, std::greater< Value > >;
 ...
 * overflow.
 * @note This capacity is supposed to be integer type.
 */
#ifdef DOXYGEN
    ...
    class NagamochiIbaraki {
      ...
      using Value = typename Traits::CapacityMap::Value;
      ...
      struct SetUnitCapacityTraits: public Traits {
        using CapacityMap = ConstMap< typename Graph::Edge, Const< int, 1 > >;
        static CapacityMap* createCapacityMap(const Graph&) { return new CapacityMap(); }
      };
       ...
       * the capacity map to a constMap<Edge, int, 1>() instance
       */
      struct SetUnitCapacity: public NagamochiIbaraki< Graph, CapacityMap, SetUnitCapacityTraits > {
        using Create = NagamochiIbaraki< Graph, CapacityMap, SetUnitCapacityTraits >;
      };
      ...
      struct SetHeap: public NagamochiIbaraki< Graph, CapacityMap, SetHeapTraits< H, CR > > {
        using Create = NagamochiIbaraki< Graph, CapacityMap, SetHeapTraits< H, CR > >;
      };
      ...
      struct SetStandardHeapTraits: public Traits {
        using HeapCrossRef = CR;
        using Heap = H;
        static HeapCrossRef* createHeapCrossRef(int size) { return new HeapCrossRef(size); }
       ...
       */
      template < class H, class CR = RangeMap< int > >
      struct SetStandardHeap: public NagamochiIbaraki< Graph, CapacityMap, SetStandardHeapTraits< H, CR > > {
        using Create = NagamochiIbaraki< Graph, CapacityMap, SetStandardHeapTraits< H, CR > >;
      };
      ...
      nt::TrivialDynamicArray< ArcData >            _arcs;
      nt::TrivialDynamicArray< EdgeData >           _edges;
      ...
      using NodeList = typename Graph::template NodeMap< typename Graph::Node >;
      NodeList* _next_rep;

      using MinCutMap = typename Graph::template NodeMap< bool >;
      MinCutMap* _cut_map;

      void createStructures() {
        if (!_nodes) { _nodes = new (typename Graph::template NodeMap< NodeData >)(_graph); }
        if (!_capacity) {
        ...
        if (!_next_rep) { _next_rep = new NodeList(_graph); }
        if (!_cut_map) { _cut_map = new MinCutMap(_graph); }
      }
      ...
      NagamochiIbaraki(const Graph& graph, const CapacityMap& capacity) :
          _graph(graph), _capacity(&capacity), _local_capacity(false), _nodes(0), _arcs(), _edges(), _min_cut(),
          _heap_cross_ref(0), _local_heap_cross_ref(false), _heap(0), _local_heap(false), _next_rep(0), _cut_map(0) {}
      ...
      NagamochiIbaraki(const Graph& graph) :
          _graph(graph), _capacity(0), _local_capacity(false), _nodes(0), _arcs(), _edges(), _min_cut(),
          _heap_cross_ref(0), _local_heap_cross_ref(false), _heap(0), _local_heap(false), _next_rep(0), _cut_map(0) {}
      ...
      void init() {
        ...
        for (typename Graph::NodeIt n(_graph); n != INVALID; ++n) {
          ...
          (*_nodes)[n].curr_arc = -1;
          (*_nodes)[n].prev = prev;
          if (prev != INVALID) { (*_nodes)[prev].next = n; }
        ...
        for (typename Graph::NodeIt n(_graph); n != INVALID; ++n) {
          for (typename Graph::OutArcIt a(_graph, n); a != INVALID; ++a) {
            ...
            if (c != -1 && _arcs[c ^ 1].target == n) {
              ...
              if ((*_nodes)[n].first_arc != -1) { _arcs[(*_nodes)[n].first_arc].prev = (index << 1); }
              ...
              if ((*_nodes)[m].first_arc != -1) { _arcs[(*_nodes)[m].first_arc].prev = ((index << 1) | 1); }
        ...
        for (typename Graph::Node n = _first_node; n != INVALID; n = (*_nodes)[n].next) {
        ...
        if (_min_cut == 0) { _first_node = INVALID; }
      }
      ...
      bool processNextPhase() {
        ...
        nt::TrivialDynamicArray< typename Graph::Node > order;
            ...
            if (!(_edges[a >> 1].cut < pmc)) {
              ...
              for (int b = (*_nodes)[m].first_arc; b != -1; b = nb) {
                ...
                if (c != -1 && _arcs[c ^ 1].target == n) {
                  ...
                  if (_edges[b >> 1].cut < _edges[c >> 1].cut) { _edges[b >> 1].cut = _edges[c >> 1].cut; }
                  if (_arcs[b ^ 1].prev != -1) {
                    _arcs[_arcs[b ^ 1].prev].next = _arcs[b ^ 1].next;
                  } else {
                    (*_nodes)[o].first_arc = _arcs[b ^ 1].next;
                  }
                  if (_arcs[b ^ 1].next != -1) { _arcs[_arcs[b ^ 1].next].prev = _arcs[b ^ 1].prev; }
                } else {
                  if (_arcs[a].next != -1) { _arcs[_arcs[a].next].prev = b; }
              ...
              if (_arcs[a].next != -1) { _arcs[_arcs[a].next].prev = _arcs[a].prev; }
              ...
              if ((*_nodes)[m].next != INVALID) { (*_nodes)[(*_nodes)[m].next].prev = (*_nodes)[m].prev; }
              --_node_num;
            }
          }
        }
       ...
       *
       * @note mc.run(s) is just a shortcut of the following code.
       * @code
       * mc.init();
       * mc.start();
       * @endcode
...
#endif   // _NT_NAGAMOCHI_IBARAKI_H_
