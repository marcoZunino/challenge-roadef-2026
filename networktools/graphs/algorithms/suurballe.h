 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (suurballe.h).
 *
...
#ifndef _NT_SUURBALLE_H_
#define _NT_SUURBALLE_H_
 ...
 */
    ...
    struct SuurballeDefaultTraits {
      ...
      using Length = typename LEN::Value;
      /** The type of the flow map. */
      using FlowMap = typename GR::template ArcMap< int >;
      /** The type of the potential map. */
      using PotentialMap = typename GR::template NodeMap< Length >;
      ...
      using HeapCrossRef = typename GR::template NodeMap< int >;

      /**
       * @brief The heap type used for internal Dijkstra computations.
       *
       * The type of the heap used for internal Dijkstra computations.
       * It must conform to the @ref nt::concepts::Heap "Heap" concept
       * and its priority type must be \c Length.
       */
      using Heap = BinHeap< Length, HeapCrossRef >;
    };
 ...
 *
 * @ref nt::Suurballe "Suurballe" implements an algorithm for
 ...
 *
 * @note For finding \e node-disjoint paths, this algorithm can be used
 ...
 */
    ...
    class Suurballe {
      ...
      using ConstArcMap = ConstMap< Arc, int >;
      using PredMap = typename GR::template NodeMap< Arc >;
      ...
      /** The type of the lengths. */
      using Length = typename TR::Length;

      /** The type of the flow map. */
      using FlowMap = typename TR::FlowMap;
      /** The type of the potential map. */
      using PotentialMap = typename TR::PotentialMap;
      ...
      /**
       * The @ref nt::SuurballeDefaultTraits "traits class" of the algorithm.
       */
      using Traits = TR;
      ...
      class ResidualDijkstra {
        ...
        PotentialMap                    _dist;
        nt::TrivialDynamicArray< Node > _proc_nodes;

        public:
        // Constructor
        ResidualDijkstra(Suurballe& srb) :
            _graph(srb._graph), _length(srb._length), _flow(*srb._flow), _pi(*srb._potential), _pred(srb._pred),
      ...
      struct SetFlowMap: public Suurballe< GR, LEN, SetFlowMapTraits< T > > {
        using Create = Suurballe< GR, LEN, SetFlowMapTraits< T > >;
      };

      template < typename T >
      struct SetPotentialMapTraits: public Traits {
        using PotentialMap = T;
      };
      ...
      struct SetPotentialMap: public Suurballe< GR, LEN, SetPotentialMapTraits< T > > {
        using Create = Suurballe< GR, LEN, SetPotentialMapTraits< T > >;
      };
      ...
      struct SetPath: public Suurballe< GR, LEN, SetPathTraits< T > > {
        using Create = Suurballe< GR, LEN, SetPathTraits< T > >;
      };
       ...
       * The heap type must conform to the @ref nt::concepts::Heap "Heap"
      ...
      struct SetHeap: public Suurballe< GR, LEN, SetHeapTraits< H, CR > > {
        using Create = Suurballe< GR, LEN, SetHeapTraits< H, CR > >;
      };
      ...
      Suurballe(const Digraph& graph, const LengthMap& length) :
          _graph(graph), _length(length), _flow(0), _local_flow(false), _potential(0), _local_potential(false),
          _pred(graph), _init_dist(0), _init_pred(0) {}
       ...
       *
       * @note Apart from the return value, <tt>s.run(s, t, k)</tt> is
      ...
      void fullInit(const Node& s) {
        // Initialize maps
        init(s);
        if (!_init_dist) { _init_dist = new PotentialMap(_graph); }
        if (!_init_pred) { _init_pred = new PredMap(_graph); }

        // Run a full Dijkstra
        typename DijkstraLegacy< Digraph, LengthMap >::template SetStandardHeap< Heap >::template SetDistMap<
           PotentialMap >::template SetPredMap< PredMap >::Create dijk(_graph, _length);
       ...
       * arc-disjoint paths found.
       *
       * @note Apart from the return value, <tt>s.start(t, k)</tt> is
       * just a shortcut of the following code.
       * @code
       * s.findFlow(t, k);
       * s.findPaths();
