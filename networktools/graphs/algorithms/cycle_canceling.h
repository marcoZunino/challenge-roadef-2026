 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (cycle_canceling.h).
 *
...
#ifndef _NT_CYCLE_CANCELING_H_
#define _NT_CYCLE_CANCELING_H_
 ...
 *
 * @note For more information about the three available methods,
 * see @ref Method.
 */
#ifdef DOXYGEN
    ...
    class CycleCanceling {
      ...
      using IntVector = nt::TrivialDynamicArray< int >;
      using DoubleVector = nt::TrivialDynamicArray< double >;
      using ValueVector = nt::TrivialDynamicArray< Value >;
      using CostVector = nt::TrivialDynamicArray< Cost >;
      using BoolVector = nt::TrivialDynamicArray< char >;
      ...
      class StaticVectorMap {
        ...
        StaticVectorMap(nt::TrivialDynamicArray< Value >& v) : _v(v) {}

        const Value& operator[](const Key& key) const { return _v[StaticDigraph::id(key)]; }

        Value& operator[](const Key& key) { return _v[StaticDigraph::id(key)]; }

        void set(const Key& key, const Value& val) { _v[StaticDigraph::id(key)] = val; }

        private:
        nt::TrivialDynamicArray< Value >& _v;
      };

      using CostNodeMap = StaticVectorMap< StaticDigraph::Node, Cost >;
      using CostArcMap = StaticVectorMap< StaticDigraph::Arc, Cost >;
      ...
      using IntPair = nt::Pair< int, int >;
      StaticDigraph                      _sgr;
      nt::TrivialDynamicArray< IntPair > _arc_vec;
      nt::TrivialDynamicArray< Cost >    _cost_vec;
      IntVector                          _id_vec;
      ...
      CycleCanceling(const GR& graph) :
          _graph(graph), _node_id(graph), _arc_idf(graph), _arc_idb(graph), _cost_map(_cost_vec), _pi_map(_pi),
          INF(std::numeric_limits< Value >::has_infinity ? std::numeric_limits< Value >::infinity()
                                                         : std::numeric_limits< Value >::max()) {
        // Check the number types
        NT_ASSERT(std::numeric_limits< Value >::is_signed, "The flow type of CycleCanceling must be signed");
        NT_ASSERT(std::numeric_limits< Cost >::is_signed, "The cost type of CycleCanceling must be signed");
      ...
      CycleCanceling& reset() {
        ...
        _arc_num = nt::graphs::countArcs(_graph);
       ...
       *
       * @note The return type of the function can be specified as a
       ...
       * @pre @ref run() must be called before using this function.
       */
      Cost potential(const Node& n) const { return static_cast< Cost >(_pi[_node_id[n]]); }
      ...
      ProblemType init() {
        ...
        NT_DEBUG(checkBoundMaps(), "Upper bounds must be greater or equal to the lower bounds");

        // Initialize vectors
        for (int i = 0; i != _res_node_num; ++i) {
          _pi[i] = 0;
        }
        ValueVector excess(nt::clone(_supply));
        ...
        using ValueArcMap = typename Digraph::template ArcMap< Value >;
        using ValueNodeMap = typename Digraph::template NodeMap< Value >;
        ...
        Circulation< Digraph, ConstMap< Arc, Value >, ValueArcMap, ValueNodeMap > circ(_graph, low, cap, sup);
      ...
      void start(Method method) {
        ...
        switch (method) {
          case SIMPLE_CYCLE_CANCELING: startSimpleCycleCanceling(); break;
          case MINIMUM_MEAN_CYCLE_CANCELING: startMinMeanCycleCanceling(); break;
          case CANCEL_AND_TIGHTEN: startCancelAndTighten(); break;
        }

        // Compute node potentials
        if (method != SIMPLE_CYCLE_CANCELING) {
          buildResidualNetwork();
          typename BellmanFord< StaticDigraph, CostArcMap >::template SetDistMap< CostNodeMap >::Create bf(_sgr,
                                                                                                           _cost_map);
          bf.distMap(_pi_map);
          bf.init(0);
          bf.start();
      ...
      void startSimpleCycleCanceling() {
        ...
        using FilterMap = StaticVectorMap< StaticDigraph::Arc, Value >;
        using ResDigraph = FilterArcs< StaticDigraph, FilterMap >;
        using PredMap = StaticVectorMap< StaticDigraph::Node, StaticDigraph::Arc >;
        using BF = typename BellmanFord< ResDigraph, CostArcMap >::template SetDistMap<
           CostNodeMap >::template SetPredMap< PredMap >::Create;
        ...
        nt::TrivialDynamicArray< int >                cycle;
        nt::TrivialDynamicArray< StaticDigraph::Arc > pred(_res_arc_num);
        ...
        while (!optimal) {
          ...
          while (!cycle_found) {
            ...
            int curr_iter_num = iter_num + length_bound <= _node_num ? length_bound : _node_num - iter_num;
            ...
            if (real_iter_num < curr_iter_num) {
              ...
              nt::TrivialDynamicArray< int > state(_res_node_num, 0);
              int                            id = 0;
              for (int u = 0; u != _res_node_num; ++u) {
                if (state[u] != 0) continue;
                ++id;
                int v = u;
                for (; v != -1 && state[v] == 0; v = pred[v] == INVALID ? -1 : rgr.id(rgr.source(pred[v]))) {
                  state[v] = id;
                }
                if (v != -1 && state[v] == id) {
            ...
            if (!cycle_found) { length_bound = static_cast< int >(length_bound * BF_LIMIT_FACTOR); }
          }
        }
      }
      ...
      void startMinMeanCycleCanceling() {
        using SPath = Path< StaticDigraph >;
        using SPathArcIt = typename SPath::ArcIt;
        using HwMmc = typename HowardMmc< StaticDigraph, CostArcMap >::template SetPath< SPath >::Create;
        using HoMmc = typename HartmannOrlinMmc< StaticDigraph, CostArcMap >::template SetPath< SPath >::Create;

        const double HW_ITER_LIMIT_FACTOR = 1.0;
        const int    HW_ITER_LIMIT_MIN_VALUE = 5;

        const int hw_iter_limit =
           std::max(static_cast< int >(HW_ITER_LIMIT_FACTOR * _node_num), HW_ITER_LIMIT_MIN_VALUE);
        ...
        buildResidualNetwork();
        while (true) {
          typename HwMmc::TerminationCause hw_tc = hw_mmc.findCycleMean(hw_iter_limit);
      ...
      void startCancelAndTighten() {
        ...
        nt::TrivialDynamicArray< int > stack(_res_node_num);
        nt::TrivialDynamicArray< int > proc_vector(_res_node_num);

        // Initialize epsilon
        double epsilon = 0;
        for (int a = 0; a != _res_arc_num; ++a) {
          if (_res_cap[a] > 0 && -_cost[a] > epsilon) epsilon = -_cost[a];
        }
        ...
        while (epsilon * _res_node_num >= 1) {
          ...
          for (int start = 0; start != _res_node_num; ++start) {
            ...
            for (; a != last_out && (_res_cap[a] == 0 || !tol.negative(_cost[a] + p - pi[_target[a]])); ++a)
              ;
            if (a == last_out) {
          ...
          if (--iter > 0) {
            ...
            for (int i = proc_head; i > 0; --i) {
              ...
              for (int a = _first_out[u]; a != last_out; ++a) {
                int v = _target[a];
                if (_res_cap[a] > 0 && tol.negative(_cost[a] + p - pi[v]) && l > level[v]) level[v] = l;
              }
            }
            ...
            using HwMmc = HowardMmc< StaticDigraph, CostArcMap >;
            using HoMmc = HartmannOrlinMmc< StaticDigraph, CostArcMap >;
            using BF = typename BellmanFord< StaticDigraph, CostArcMap >::template SetDistMap< CostNodeMap >::Create;
