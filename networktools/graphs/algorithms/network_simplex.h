 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (network_simplex.h).
 *
...
#ifndef _NT_NETWORK_SIMPLEX_H_
#define _NT_NETWORK_SIMPLEX_H_
...
namespace nt {
  namespace graphs {
     ...
     * be integer.
     *
     * @note %NetworkSimplex provides five different pivot rule
    ...
    class NetworkSimplex {
      ...
      using CharVector = nt::TrivialDynamicArray< signed char >;
      // Note: TrivialDynamicArray<signed char> is used instead of TrivialDynamicArray<ArcState> and
      // TrivialDynamicArray<ArcDirection> for efficiency reasons
      ...
      class FirstEligiblePivotRule {
        ...
        FirstEligiblePivotRule(NetworkSimplex& ns) :
            _source(ns._source), _target(ns._target), _cost(ns._cost), _state(ns._state), _pi(ns._pi),
      ...
      class AlteringListPivotRule {
        ...
        AlteringListPivotRule(NetworkSimplex& ns) :
            _source(ns._source), _target(ns._target), _cost(ns._cost), _state(ns._state), _pi(ns._pi),
            _in_arc(ns.in_arc), _search_arc_num(ns._search_arc_num), _next_arc(0), _cand_cost(ns._search_arc_num),
        ...
        search_end:
          ...
          std::partial_sort(
             _candidates.begin(), _candidates.begin() + new_length, _candidates.begin() + _curr_length, _sort_func);
      ...
      NetworkSimplex(const GR& graph, bool arc_mixing = true) :
        ...
        NT_ASSERT(std::numeric_limits< Value >::is_signed, "The flow type of NetworkSimplex must be signed");
        NT_ASSERT(std::numeric_limits< Cost >::is_signed, "The cost type of NetworkSimplex must be signed");
      ...
      bool initialPivots() {
        Value                           curr, total = 0;
        nt::TrivialDynamicArray< Node > supply_nodes, demand_nodes;
        ...
        if (_sum_supply >= 0) {
          if (supply_nodes.size() == 1 && demand_nodes.size() == 1) {
            ...
            nt::TrivialDynamicArray< Node >       stack;
            reached[t] = true;
            stack.push_back(t);
            while (!stack.empty()) {
              Node u, v = stack.back();
            ...
            for (int i = 0; i != int(demand_nodes.size()); ++i) {
              ...
              if (min_arc != INVALID) { arc_vector.push_back(_arc_id[min_arc]); }
            }
          }
        } else {
