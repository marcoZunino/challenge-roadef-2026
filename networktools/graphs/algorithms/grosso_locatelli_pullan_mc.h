 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (grosso_locatelli_pullan_mc.h).
 *
 ...
 * @ingroup approx_algs
 * @file
 * @brief The iterated local search algorithm of Grosso, Locatelli, and Pullan for the maximum clique problem
 *
 * @author LEMON contributors (see original source) and Morgan Chopin (morgan.chopin@orange.com)
 */

#ifndef _NT_GROSSO_LOCATELLI_PULLAN_MC_H_
#define _NT_GROSSO_LOCATELLI_PULLAN_MC_H_
...
namespace nt {
  namespace graphs {
     ...
     *
     * @note %GrossoLocatelliPullanMc provides three different node selection
    ...
    class GrossoLocatelliPullanMc {
      ...
      using BoolMatrix = nt::TrivialMultiDimArray< char, 2 >;
      ...
      class RandomSelectionRule {
        ...
        RandomSelectionRule(GrossoLocatelliPullanMc& mc) :
            _clique(mc._clique), _delta(mc._delta), _tabu(mc._tabu), _rnd(mc._rnd), _n(mc._n) {}
      ...
      class DegreeBasedSelectionRule {
        ...
        DegreeBasedSelectionRule(GrossoLocatelliPullanMc& mc) :
            _clique(mc._clique), _delta(mc._delta), _tabu(mc._tabu), _rnd(mc._rnd), _n(mc._n), _deg(_n) {
          for (int i = 0; i != _n; i++) {
            int                   d = 0;
            nt::ArrayView< char > row = mc._gr.row(i);
            for (int j = 0; j != _n; j++) {
              if (row[j]) d++;
            }
            _deg[i] = d;
      ...
      class PenaltyBasedSelectionRule {
        ...
        PenaltyBasedSelectionRule(GrossoLocatelliPullanMc& mc) :
            _clique(mc._clique), _delta(mc._delta), _tabu(mc._tabu), _rnd(mc._rnd), _n(mc._n), _penalty(_n, 0) {}
       ...
       */
      GrossoLocatelliPullanMc(const GR& graph, int seed = 0) : _graph(graph), _id(_graph), _rnd(seed) { initOptions(); }
       ...
       * algorithm.
       */
      GrossoLocatelliPullanMc(const GR& graph, const Random& random) : _graph(graph), _id(_graph), _rnd(random) {
        initOptions();
      }
      ...
      TerminationCause run(SelectionRule rule = PENALTY_BASED) {
        init();
        switch (rule) {
          case RANDOM: return start< RandomSelectionRule >();
          case DEGREE_BASED: return start< DegreeBasedSelectionRule >();
          default: return start< PenaltyBasedSelectionRule >();
        }
      }
      ...
      void addCliqueNode(int u) {
        ...
        _clique[u] = true;
        _size++;
        nt::ArrayView< char > row = _gr.row(u);
        for (int i = 0; i != _n; i++) {
          if (!row[i]) _delta[i]++;
        }
      }
      ...
      void init() {
        _n = nt::graphs::countNodes(_graph);
        int ui = 0;
        for (NodeIt u(_graph); u != INVALID; ++u) {
          _id[u] = ui++;
        }
        if (_n > 0) {
          _gr.setDimensions(_n, _n);
          _gr.setBitsToZero();
        }
        ui = 0;
        for (NodeIt u(_graph); u != INVALID; ++u) {
      ...
      TerminationCause start() {
        ...
        const int max_restart = _iteration_limit >= 0 ? _iteration_limit : std::numeric_limits< int >::max();
        const int max_select = _step_limit >= 0 ? _step_limit : std::numeric_limits< int >::max();
        ...
        while (select < max_select && restart < max_restart) {
          ...
          if (_delta_based_restart) {
            restart_nodes.clear();
            for (int i = 0; i != _n; i++) {
              if (_delta[i] >= _restart_delta_limit) restart_nodes.push_back(i);
            }
          }
          int rs_node = -1;
          ...
          nt::ArrayView< char > row = _gr.row(rs_node);
          for (int i = 0; i != _n; i++) {
