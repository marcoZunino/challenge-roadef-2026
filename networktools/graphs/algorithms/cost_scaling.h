 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (cost_scaling.h).
 *
...
#ifndef _NT_COST_SCALING_H_
#define _NT_COST_SCALING_H_
...
namespace nt {
  namespace graphs {
     ...
     * @tparam C The number type used for costs and potentials.
     * By default it is the same as \c V.
     */
    template < typename GR, typename V = int, typename C = V, bool integer = std::numeric_limits< C >::is_integer >
     ...
     *
     * @note %CostScaling provides three different internal methods,
     * from which the most efficient one is used by default.
     * For more information, see @ref Method.
     */
    template < typename GR, typename V = int, typename C = V, typename TR = CostScalingDefaultTraits< GR, V, C > >
    class CostScaling {
      public:
      /** The type of the digraph */
      ...
      /**
       * @brief The @ref nt::CostScalingDefaultTraits "traits class"
       * of the algorithm
       */
      using Traits = TR;
      ...
      using LargeCostVector = nt::TrivialDynamicArray< LargeCost >;
      using BoolVector = nt::BoolDynamicArray;
      // Note: TrivialDynamicArray<char> is used instead of TrivialDynamicArray<bool>
      ...
      using LargeCostArcMap = StaticVectorMap< StaticDigraph::Arc, LargeCost >;
      ...
      struct SetLargeCost: public CostScaling< GR, V, C, SetLargeCostTraits< T > > {
        using Create = CostScaling< GR, V, C, SetLargeCostTraits< T > >;
      };
      ...
      CostScaling(const GR& graph) :
        ...
        static_assert(std::numeric_limits< Value >::is_signed, "The flow type of CostScaling must be signed");
        static_assert(std::numeric_limits< Cost >::is_signed, "The cost type of CostScaling must be signed");
      ...
      void start(Method method) {
        ...
        switch (method) {
          case PUSH: startPush(); break;
          case AUGMENT: startAugment(_res_node_num - 1); break;
          case PARTIAL_AUGMENT: startAugment(MAX_PARTIAL_PATH_LENGTH); break;
        }
        ...
        if (!optimal) {
          ...
          StaticDigraph                        sgr;
          nt::TrivialDynamicArray< IntPair >   arc_vec;
          nt::TrivialDynamicArray< LargeCost > cost_vec;
          ...
          typename BellmanFord< StaticDigraph, LargeCostArcMap >::Create bf(sgr, cost_map);
          bf.init(0);
          bf.start();
              ...
              if (_res_cap[a] > 0) {
                ...
                if (rc < 0) {
                  ...
                  if (nrc < LargeCost(_max_rank)) {
                    ...
                    if (new_rank_v > _rank[v]) { _rank[v] = new_rank_v; }
                  }
                }
              }
            }
                ...
                if (_res_cap[a] > 0) {
                  ...
                  if (old_rank_v < rank) {
                    ...
                    if (rc < 0) {
                      ...
                      new_rank_v = 0;
                      if (nrc < LargeCost(_max_rank)) { new_rank_v = rank - 1 - static_cast< int >(nrc); }
                    }
              ...
              if (_res_cap[a] > 0) {
                ...
                if (_cost[a] + pi_tip - _pi[v] < 0) {
                  if (!reached[v]) {
                    ...
                    if (cycle_cnt >= MAX_CYCLE_CANCEL) { return false; }
            ...
            for (int a = _first_out[u]; a != last_out; ++a) {
              ...
              if (_res_cap[ra] > 0) {
                ...
                if (r < old_rank_v) {
                  ...
                  if (nrc < LargeCost(_max_rank)) { new_rank_v = r + 1 + static_cast< int >(nrc); }
      ...
      void startAugment(int max_length) {
        ...
        const int    global_update_skip =
           static_cast< int >(GLOBAL_UPDATE_FACTOR * (_res_node_num + _sup_node_num * _sup_node_num));
        ...
        int        relabel_cnt = 0;
        int        eps_phase_cnt = 0;
        for (; _epsilon >= 1; _epsilon = _epsilon < _alpha && _epsilon > 1 ? 1 : _epsilon / _alpha) {
          ++eps_phase_cnt;
          ...
          while (true) {
            ...
            while (_active_nodes.size() > 0 && _excess[_active_nodes.front()] <= 0) {
            ...
            while (int(path.size()) < max_length && _excess[tip] >= 0) {
              ...
              if (tip != start) {
                ...
                min_red_cost = std::min(min_red_cost, _cost[ra] + pi_tip - _pi[_target[ra]]);
              }
              last_out = _next_out[tip];
              for (int a = _first_out[tip]; a != last_out; ++a) {
                if (_res_cap[a] > 0) {
                  rc = _cost[a] + pi_tip - _pi[_target[a]];
                  if (rc < min_red_cost) { min_red_cost = rc; }
                }
              }
          ...
          augment:
            ...
            for (int i = 0; i != int(path.size()); ++i) {
              ...
              _excess[u] -= delta;
              _excess[v] += delta;
              if (_excess[v] > 0 && _excess[v] <= delta) { _active_nodes.push_back(v); }
            }
            path.clear();
      ...
      void startPush() {
        ...
        // Perform cost scaling phases
        BoolVector      is_hyper(_res_node_num, false);
        ...
        for (; _epsilon >= 1; _epsilon = _epsilon < _alpha && _epsilon > 1 ? 1 : _epsilon / _alpha) {
          ...
          next_node:
              ...
              for (a = _next_out[n]; a != last_out; ++a) {
                if (_res_cap[a] > 0 && _cost[a] + pi_n - _pi[_target[a]] < 0) {
                  ...
                  for (int ta = _next_out[t]; ta != last_out_t; ++ta) {
                    if (_res_cap[ta] > 0 && _cost[ta] + pi_t - _pi[_target[ta]] < 0) ahead += _res_cap[ta];
                    if (ahead >= delta) break;
                  }
                  if (ahead < 0) ahead = 0;

                  // Push flow along the arc
                  if (ahead < delta && !is_hyper[t]) {
                    _res_cap[a] -= ahead;
                    ...
                    _excess[n] -= delta;
                    _excess[t] += delta;
                    if (_excess[t] > 0 && _excess[t] <= delta) _active_nodes.push_back(t);
                  }
            ...
            if (_excess[n] > 0 || is_hyper[n]) {
              min_red_cost = is_hyper[n] ? -hyper_cost[n] : std::numeric_limits< LargeCost >::max();
