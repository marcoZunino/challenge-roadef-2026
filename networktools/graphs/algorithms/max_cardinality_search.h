 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (max_cardinality_search.h).
 *
...
#ifndef _NT_MAX_CARDINALITY_SEARCH_H_
#define _NT_MAX_CARDINALITY_SEARCH_H_
  ...
  namespace graphs {
    ...
    struct MaxCardinalitySearchDefaultTraits {
      ...
      struct CapMapSelector {
        ...
        static CapacityMap* createCapacityMap(const Digraph& g) { return new CapacityMap(g); }
      };

      template < typename CM >
      struct CapMapSelector< ConstMap< CM, Const< int, 1 > > > {
        using CapacityMap = ConstMap< CM, Const< int, 1 > >;

        static CapacityMap* createCapacityMap(const Digraph&) { return new CapacityMap; }
      };
       ...
       */
      using CapacityMap = typename CapMapSelector< CAP >::CapacityMap;
       ...
       * HeapCrossRef.
       */
      static HeapCrossRef* createHeapCrossRef(const Digraph& digraph) { return new HeapCrossRef(digraph); }

      template < typename CapacityMap >
      struct HeapSelector {
        template < typename Value, typename Ref >
        struct Selector {
          using Heap = BinHeap< Value, Ref, std::greater< Value > >;
        };
      };
      ...
      struct HeapSelector< ConstMap< CapacityKey, Const< int, 1 > > > {
        template < typename Value, typename Ref >
        struct Selector {
          using Heap = BucketHeap< Ref, false >;
        };
      };
       ...
       */
      using Heap = typename HeapSelector< CapacityMap >::template Selector< Value, HeapCrossRef >::Heap;
       ...
       * It must meet the @ref concepts::WriteMap "WriteMap" concept.
       */
      using CardinalityMap = typename Digraph::template NodeMap< Value >;
       ...
       */
      static CardinalityMap* createCardinalityMap(const Digraph& digraph) { return new CardinalityMap(digraph); }
    };
    ...
    class MaxCardinalitySearch {
      ...
      using ProcessedMap = typename Traits::ProcessedMap;
      /**
       * The type of the map that stores the cardinalities of the nodes.
       */
      using CardinalityMap = typename Traits::CardinalityMap;
      ...
      public:
      using Create = MaxCardinalitySearch;
      ...
      struct DefCapacityMapTraits: public Traits {
        ...
        static CapacityMap* createCapacityMap(const Digraph&) {
          NT_ASSERT(false, "Uninitialized parameter.");
          return 0;
        }
      };
      /**
       ...
       */
      template < class T >
      struct SetCapacityMap: public MaxCardinalitySearch< Digraph, CapacityMap, DefCapacityMapTraits< T > > {
        using Create = MaxCardinalitySearch< Digraph, CapacityMap, DefCapacityMapTraits< T > >;
      };

      template < class T >
      struct DefCardinalityMapTraits: public Traits {
        using CardinalityMap = T;
       ...
       */
      template < class T >
      struct SetCardinalityMap: public MaxCardinalitySearch< Digraph, CapacityMap, DefCardinalityMapTraits< T > > {
        using Create = MaxCardinalitySearch< Digraph, CapacityMap, DefCardinalityMapTraits< T > >;
      };
       ...
       */
      template < class T >
      struct SetProcessedMap: public MaxCardinalitySearch< Digraph, CapacityMap, DefProcessedMapTraits< T > > {
        using Create = MaxCardinalitySearch< Digraph, CapacityMap, DefProcessedMapTraits< T > >;
      };
      ...
      struct SetHeap: public MaxCardinalitySearch< Digraph, CapacityMap, DefHeapTraits< H, CR > > {
        using Create = MaxCardinalitySearch< Digraph, CapacityMap, DefHeapTraits< H, CR > >;
      };
       ...
       */
      template < class H, class CR = typename Digraph::template NodeMap< int > >
      struct SetStandardHeap: public MaxCardinalitySearch< Digraph, CapacityMap, DefStandardHeapTraits< H, CR > > {
        using Create = MaxCardinalitySearch< Digraph, CapacityMap, DefStandardHeapTraits< H, CR > >;
      };
      ...
      MaxCardinalitySearch(const Digraph& digraph, const CapacityMap& capacity) :
          _graph(&digraph), _capacity(&capacity), local_capacity(false), _cardinality(0), local_cardinality(false),
          _processed(0), local_processed(false), _heap_cross_ref(0), local_heap_cross_ref(false), _heap(0),
          local_heap(false) {}
      ...
      MaxCardinalitySearch(const Digraph& digraph) :
          _graph(&digraph), _capacity(0), local_capacity(false), _cardinality(0), local_cardinality(false),
          _processed(0), local_processed(false), _heap_cross_ref(0), local_heap_cross_ref(false), _heap(0),
          local_heap(false) {}

      /** @brief Destructor. */
      ~MaxCardinalitySearch() noexcept {
      ...
      void addSource(Node source, Value capacity = 0) {
        if (_heap->state(source) == Heap::PRE_HEAP) { _heap->push(source, capacity); }
      }
      ...
      Node processNextNode() {
        ...
        for (InArcIt it(*_graph, node); it != INVALID; ++it) {
          ...
          switch (_heap->state(source)) {
            case Heap::PRE_HEAP: _heap->push(source, (*_capacity)[it]); break;
            case Heap::IN_HEAP: _heap->decrease(source, (*_heap)[source] + (*_capacity)[it]); break;
...
#endif   // _NT_MAX_CARDINALITY_SEARCH_H_
