 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (howard_mmc.h).
 *
...
#ifndef _NT_HOWARD_MMC_H_
#define _NT_HOWARD_MMC_H_
 ...
 */
    ...
    class HowardMmc {
       ...
       *
       * The path type of the found cycles.
       * Using the @ref nt::HowardMmcDefaultTraits "default traits class",
       * it is @ref nt::Path "Path<Digraph>".
       */
      using Path = typename TR::Path;

      /**
       * The @ref nt::HowardMmcDefaultTraits "traits class" of the algorithm
       */
      using Traits = TR;
      ...
      nt::DynamicArray< nt::TrivialDynamicArray< Node > >                  _comp_nodes;
      nt::TrivialDynamicArray< Node >*                                     _nodes;
      typename Digraph::template NodeMap< nt::TrivialDynamicArray< Arc > > _in_arcs;
      ...
      struct SetLargeCost: public HowardMmc< GR, CM, SetLargeCostTraits< T > > {
        using Create = HowardMmc< GR, CM, SetLargeCostTraits< T > >;
      };
      ...
      struct SetPath: public HowardMmc< GR, CM, SetPathTraits< T > > {
        using Create = HowardMmc< GR, CM, SetPathTraits< T > >;
      };
      ...
      HowardMmc(const Digraph& digraph, const CostMap& cost) :
          _gr(digraph), _cost(cost), _best_found(false), _best_cost(0), _best_size(1), _cycle_path(NULL),
          _local_path(false), _policy(digraph), _reached(digraph), _level(digraph), _dist(digraph), _comp(digraph),
          _in_arcs(digraph),
       ...
       *
       * @note The algorithm calls only the @ref nt::Path::addBack()
       ...
       * using this function.
       */
      double cycleMean() const { return static_cast< double >(_best_cost) / _best_size; }
      ...
      void init() {
        ...
        _queue.resize(nt::graphs::countNodes(_gr));
        _best_found = false;
        _best_cost = 0;
        _best_size = 1;
        _cycle_path->clear();
      ...
      bool buildPolicyGraph(int comp) {
        ...
        if (_nodes->size() < 1 || (_nodes->size() == 1 && _in_arcs[(*_nodes)[0]].size() == 0)) { return false; }
