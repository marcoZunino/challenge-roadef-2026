 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (hartmann_orlin_mmc.h).
 *
...
#ifndef _NT_HARTMANN_ORLIN_MMC_H_
#define _NT_HARTMANN_ORLIN_MMC_H_
 ...
 */
    ...
    class HartmannOrlinMmc {
       ...
       *
       * The path type of the found cycles.
       * Using the @ref nt::HartmannOrlinMmcDefaultTraits
      ...
      /**
       * @brief The
       * @ref nt::HartmannOrlinMmcDefaultTraits "traits class"
       * of the algorithm
       */
      using Traits = TR;
      ...
      struct SetLargeCost: public HartmannOrlinMmc< GR, CM, SetLargeCostTraits< T > > {
        using Create = HartmannOrlinMmc< GR, CM, SetLargeCostTraits< T > >;
      };
      ...
      struct SetPath: public HartmannOrlinMmc< GR, CM, SetPathTraits< T > > {
        using Create = HartmannOrlinMmc< GR, CM, SetPathTraits< T > >;
      };
      ...
      HartmannOrlinMmc(const Digraph& digraph, const CostMap& cost) :
          _gr(digraph), _cost(cost), _comp(digraph), _out_arcs(digraph), _best_found(false), _best_cost(0),
          _best_size(1), _cycle_path(NULL), _local_path(false), _data(digraph),
          INF(std::numeric_limits< LargeCost >::has_infinity ? std::numeric_limits< LargeCost >::infinity()
                                                             : std::numeric_limits< LargeCost >::max()) {}

      /** Destructor. */
      ~HartmannOrlinMmc() noexcept {
       ...
       *
       * @note The algorithm calls only the @ref nt::Path::addFront()
      ...
      bool checkTermination(int k) {
        ...
        for (int i = 0; i < n; ++i) {
          ...
          for (int j = k; j >= 0; --j) {
            ...
            level[u] = Pair(i, j);
            if (j != 0) { u = _gr.source(_data[u][j].pred); }
          }
        }
        ...
        if (_curr_found && k < n) {
          ...
          for (ArcIt a(_gr); a != INVALID; ++a) {
            if (_tolerance.less(_cost[a] * _curr_size - _curr_cost, pi[_gr.target(a)] - pi[_gr.source(a)])) {
              done = false;
              break;
            }
          }
...
#endif   // _NT_HARTMANN_ORLIN_MMC_H_