 ...
 * ============================================================================
 *
 * This file incorporates work from the LEMON graph library (elevator.h).
 *
...
#ifndef _NT_ELEVATOR_H_
#define _NT_ELEVATOR_H_
...
namespace nt {
  namespace graphs {
    ...
    class Elevator {
      ...
      private:
      using Vit = Item*;
      using VitMap = typename ItemSetTraits< GR, Item >::template Map< Vit >::Type;
      using IntMap = typename ItemSetTraits< GR, Item >::template Map< int >::Type;
      ...
      VitMap                          _where;
      IntMap                          _level;
      nt::TrivialDynamicArray< Item > _items;
      nt::TrivialDynamicArray< Vit >  _first;
      nt::TrivialDynamicArray< Vit >  _last_active;
      ...
      Elevator(const GR& graph, int max_level) :
          _g(graph), _max_level(max_level), _item_num(_max_level), _where(graph), _level(graph, 0), _items(_max_level),
          _first(_max_level + 2), _last_active(_max_level + 2), _highest_active(-1) {}
      /** Constructor. */
      ...
      Elevator(const GR& graph) :
          _g(graph), _max_level(countItems< GR, Item >(graph)), _item_num(_max_level), _where(graph), _level(graph, 0),
          _items(_max_level), _first(_max_level + 2), _last_active(_max_level + 2), _highest_active(-1) {}
      ...
      void deactivate(Item i) {
        ...
        while (_highest_active >= 0 && _last_active[_highest_active] < _first[_highest_active])
          _highest_active--;
      }
       ...
       * item.
       */
      Item highestActive() const { return _highest_active >= 0 ? *_last_active[_highest_active] : INVALID; }
       ...
       */
      Item activeOn(int l) const { return _last_active[l] >= _first[l] ? *_last_active[l] : INVALID; }
      ...
      void liftToTop(int l) {
        ...
        for (_highest_active = l - 1; _highest_active >= 0 && _last_active[_highest_active] < _first[_highest_active];
             _highest_active--)
          ;
      }
    ...
    class LinkedElevator {
      ...
      private:
      using ItemMap = typename ItemSetTraits< GR, Item >::template Map< Item >::Type;
      using IntMap = typename ItemSetTraits< GR, Item >::template Map< int >::Type;
      using BoolMap = typename ItemSetTraits< GR, Item >::template Map< bool >::Type;

      const GR&                       _graph;
      int                             _max_level;
      int                             _item_num;
      nt::TrivialDynamicArray< Item > _first, _last;
      ItemMap                         _prev, _next;
      int                             _highest_active;
      IntMap                          _level;
      BoolMap                         _active;
      ...
      LinkedElevator(const GR& graph, int max_level) :
          _graph(graph), _max_level(max_level), _item_num(_max_level), _first(_max_level + 1), _last(_max_level + 1),
          _prev(graph), _next(graph), _highest_active(-1), _level(graph), _active(graph) {}
      ...
      LinkedElevator(const GR& graph) :
          _graph(graph), _max_level(countItems< GR, Item >(graph)), _item_num(_max_level), _first(_max_level + 1),
          _last(_max_level + 1), _prev(graph, INVALID), _next(graph, INVALID), _highest_active(-1), _level(graph),
          _active(graph) {}
      ...
      void activate(Item i) {
        ...
        int level = _level[i];
        if (level > _highest_active) { _highest_active = level; }
      ...
      void deactivate(Item i) {
        ...
        if (_next[i] == INVALID || !_active[_next[i]]) goto find_highest_level;
      ...
      /** Return true if there is no active item on level \c l. */
      bool activeFree(int l) const { return _first[l] == INVALID || !_active[_first[l]]; }
       ...
       * item.
       */
      Item highestActive() const { return _highest_active >= 0 ? _first[_highest_active] : INVALID; }
       ...
       * an item. (\c l must be from the range [0...\c max_level].
       */
      Item activeOn(int l) const { return _active[_first[l]] ? _first[l] : INVALID; }
      ...
      Item liftActiveOn(int l) {
        ...
        if (_highest_active < l) { _highest_active = l; }
      }
      ...
      void lift(Item i, int new_level) {
        ...
        if (_highest_active < new_level) { _highest_active = new_level; }
      }
      ...
      void initStart() {
        ...
        _init_level = 0;
        for (typename ItemSetTraits< GR, Item >::ItemIt i(_graph); i != INVALID; ++i) {
          _level[i] = _max_level;
          _active[i] = false;
        }
      }
